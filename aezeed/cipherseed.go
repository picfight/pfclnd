...
const (
	...
	// for the Picfightcoin Days Genesis timestamp, and 16 bytes for entropy. It
	...
	// addresses, the 2 byte timestamp a BDG (Picfightcoin Days Genesis) offset,
...
var (
	// PicfightcoinGenesisDate is the timestamp of Picfightcoin's genesis block.
	...
	// the genesis date. We refer to this time period as ABE (after Picfightcoin
	// era).
	PicfightcoinGenesisDate = time.Unix(1231006505, 0)
)
...
type CipherSeed struct {
	...
	// the number of days since the timestamp in the Picfightcoin genesis block.
...
func New(internalVersion uint8, entropy *[EntropySize]byte,
	...
	// subtract that from the Picfightcoin Genesis Date. We'll then convert that
	// value to days.
	birthday := uint16(now.Sub(PicfightcoinGenesisDate) / (time.Hour * 24))
...
func (c *CipherSeed) BirthdayTime() time.Time {
	...
	return PicfightcoinGenesisDate.Add(offset)
}
