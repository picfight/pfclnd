...
func TestConstrainedPrefAttachmentNeedMoreChan(t *testing.T) {
	...
	const (
		minChanSize = 0
		maxChanSize = pfcutil.Amount(pfcutil.SatoshiPerPicfightcoin)
	...
	testCases := []struct {
		channels  []Channel
		walletAmt pfcutil.Amount

		needMore     bool
		amtAvailable pfcutil.Amount
		numMore      uint32
	}{
		...
		// channels.
		{
			[]Channel{
				{
					ChanID:   randChanID(),
					Capacity: pfcutil.Amount(prand.Int31()),
				},
				{
					ChanID:   randChanID(),
					Capacity: pfcutil.Amount(prand.Int31()),
			...
			pfcutil.Amount(pfcutil.SatoshiPerPicfightcoin * 10),
			false,
			0,
			0,
		},
		...
		// threshold.
		{
			[]Channel{
				{
					ChanID:   randChanID(),
					Capacity: pfcutil.Amount(pfcutil.SatoshiPerPicfightcoin),
				},
				{
					ChanID:   randChanID(),
					Capacity: pfcutil.Amount(pfcutil.SatoshiPerPicfightcoin),
			...
			pfcutil.Amount(pfcutil.SatoshiPerPicfightcoin * 2),
			false,
			0,
			0,
		},
		...
		// threshold. We have 10 PFC allocated amongst channels and
		// funds, atm. We're targeting 50%, so 5 PFC should be
		// allocated. Only 1 PFC is atm, so 4 PFC should be
		...
		// limit is 3.
		{
			...
			pfcutil.Amount(pfcutil.SatoshiPerPicfightcoin * 9),
			true,
			pfcutil.Amount(pfcutil.SatoshiPerPicfightcoin * 4),
			2,
		},

		// Ratio of funds in channels and total funds is below the
		// threshold. We have 14 PFC total amongst the wallet's
		// balance, and our currently opened channels. Since we're
		// targeting a 50% allocation, we should commit 7 PFC. The
		// current channels commit 4 PFC, so we should expected 3 PFC
		...
		// channel as the limit is 3.
		{
			[]Channel{
				...
				{
					ChanID:   randChanID(),
					Capacity: pfcutil.Amount(pfcutil.SatoshiPerPicfightcoin * 3),
				},
			},
			pfcutil.Amount(pfcutil.SatoshiPerPicfightcoin * 10),
			true,
			pfcutil.Amount(pfcutil.SatoshiPerPicfightcoin * 3),
			1,
		},
		...
		// threshold.
		{
			...
			pfcutil.Amount(pfcutil.SatoshiPerPicfightcoin),
			false,
			0,
			0,
		},
...
type testGraph interface {
	...
	addRandChannel(*pfcec.PublicKey, *pfcec.PublicKey,
		pfcutil.Amount) (*ChannelEdge, *ChannelEdge, error)

	addRandNode() (*pfcec.PublicKey, error)
}
...
func TestConstrainedPrefAttachmentSelectEmptyGraph(t *testing.T) {
	...
	for _, graph := range chanGraphs {
		success := t.Run(graph.name, func(t1 *testing.T) {
			...
			const walletFunds = pfcutil.SatoshiPerPicfightcoin
...
func completeGraph(t *testing.T, g testGraph, numNodes int) {
	const chanCapacity = pfcutil.SatoshiPerPicfightcoin
	nodes := make(map[int]*pfcec.PublicKey)
	for i := 0; i < numNodes; i++ {
		for j := i + 1; j < numNodes; j++ {
			...
			if node1 == nil {
				...
				nodes[i], err = pfcec.ParsePubKey(
					pubKeyBytes[:], pfcec.S256(),
				)
				if err != nil {
...
func TestConstrainedPrefAttachmentSelectTwoVertexes(t *testing.T) {
	...
	for _, graph := range chanGraphs {
		success := t.Run(graph.name, func(t1 *testing.T) {
			...
			const walletFunds = pfcutil.SatoshiPerPicfightcoin * 10
...
func TestConstrainedPrefAttachmentSelectGreedyAllocation(t *testing.T) {
	...
	for _, graph := range chanGraphs {
		success := t.Run(graph.name, func(t1 *testing.T) {
			...
			peerPub, err := pfcec.ParsePubKey(
				peerPubBytes[:], pfcec.S256(),
			)
			if err != nil {
			...
			// wallet balance to be 5.5 PFC. We're shooting for a
			// 50/50 allocation, and have 3 PFC in channels. As a
			// result, the heuristic should try to greedily
			// allocate funds to channels.
			const availableBalance = pfcutil.SatoshiPerPicfightcoin * 2.5
			scores, err := prefAttach.NodeScores(graph, nil,
			...
			// only 0.5 PFC left. That should leave us with channel
			// candidates of that size.
			const remBalance = pfcutil.SatoshiPerPicfightcoin * 0.5
			scores, err = prefAttach.NodeScores(graph, nil,
				remBalance, nodes)
			if err != nil {
				t1.Fatalf("unable to select attachment "+
