...
justiceTxBroadcast:
	...
	case _, ok := <-confChan.Confirmed:
		...
		var totalFunds, revokedFunds pfcutil.Amount
...
func (bo *breachedOutput) Amount() pfcutil.Amount {
	return bo.amt
}
...
func (b *breachArbiter) sweepSpendableOutputsTxn(txWeight int64,
	...
	var totalAmt pfcutil.Amount
	for _, input := range inputs {
		totalAmt += pfcutil.Amount(input.SignDesc().Output.Value)
	}
...
func (bo *breachedOutput) Decode(r io.Reader) error {
	...
	bo.amt = pfcutil.Amount(binary.BigEndian.Uint64(scratch[:8]))
