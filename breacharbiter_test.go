...
func initBreachedOutputs() error {
	for i := range breachedOutputs {
		...
		pubkey, err := pfcec.ParsePubKey(breachKeys[i], pfcec.S256())
		if err != nil {
...
func createTestArbiter(t *testing.T, contractBreaches chan *ContractBreachEvent,
	...
	aliceKeyPriv, _ := pfcec.PrivKeyFromBytes(pfcec.S256(),
		alicesPrivKey)
...
// createInitChannels creates two initialized test channels funded with 10 PFC,
// with 5 PFC allocated to each side. Within the channel, Alice is the
// initiator.
func createInitChannels(revocationWindow int) (*lnwallet.LightningChannel, *lnwallet.LightningChannel, func(), error) {

	aliceKeyPriv, aliceKeyPub := pfcec.PrivKeyFromBytes(pfcec.S256(),
		alicesPrivKey)
	bobKeyPriv, bobKeyPub := pfcec.PrivKeyFromBytes(pfcec.S256(),
		bobsPrivKey)
