...
const (
	...
	// exact same string for this value, along with prologue of the Picfightcoin
...
func ecdh(pub *pfcec.PublicKey, priv *pfcec.PrivateKey) []byte {
	s := &pfcec.PublicKey{}
	x, y := pfcec.S256().ScalarMult(pub.X, pub.Y, priv.D.Bytes())
	s.X = x
	s.Y = y
...
type handshakeState struct {
	...
	localEphemeral *pfcec.PrivateKey

	remoteStatic    *pfcec.PublicKey
	remoteEphemeral *pfcec.PublicKey
}
...
func newHandshakeState(initiator bool, prologue []byte,
	localPub *pfcec.PrivateKey, remotePub *pfcec.PublicKey) handshakeState {
...
func EphemeralGenerator(gen func() (*pfcec.PrivateKey, error)) func(*Machine) {
...
type Machine struct {
	...
	ephemeralGen func() (*pfcec.PrivateKey, error)
...
// initialization.
func NewBrontideMachine(initiator bool, localPub *pfcec.PrivateKey,
	remotePub *pfcec.PublicKey, options ...func(*Machine)) *Machine {
	...
	m.ephemeralGen = func() (*pfcec.PrivateKey, error) {
		return pfcec.NewPrivateKey(pfcec.S256())
	}
...
func (b *Machine) RecvActOne(actOne [ActOneSize]byte) error {
	...
	// e
	b.remoteEphemeral, err = pfcec.ParsePubKey(e[:], pfcec.S256())
	if err != nil {
		return err
	}
...
func (b *Machine) RecvActThree(actThree [ActThreeSize]byte) error {
	...
	b.remoteStatic, err = pfcec.ParsePubKey(remotePub, pfcec.S256())
	if err != nil {
		return err
	}
