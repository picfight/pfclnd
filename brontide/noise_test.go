...
func makeListener() (*Listener, *lnwire.NetAddress, error) {
	...
	localPriv, err := pfcec.NewPrivateKey(pfcec.S256())
	if err != nil {
		return nil, nil, err
	}
...
func establishTestConnection() (net.Conn, net.Conn, func(), error) {
	...
	// within our test.
	remotePriv, err := pfcec.NewPrivateKey(pfcec.S256())
	if err != nil {
		return nil, nil, nil, err
	}
...
func TestBolt0008TestVectors(t *testing.T) {
	...
	initiatorPriv, _ := pfcec.PrivKeyFromBytes(pfcec.S256(),
		initiatorKeyBytes)
	...
	responderPriv, responderPub := pfcec.PrivKeyFromBytes(pfcec.S256(),
		responderKeyBytes)
	...
	initiatorEphemeral := EphemeralGenerator(func() (*pfcec.PrivateKey, error) {
		...
		priv, _ := pfcec.PrivKeyFromBytes(pfcec.S256(), eBytes)
		return priv, nil
	})
	responderEphemeral := EphemeralGenerator(func() (*pfcec.PrivateKey, error) {
