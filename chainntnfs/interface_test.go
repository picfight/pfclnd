...
import (
	...
	// Required to auto-register the picfightcoind backed ChainNotifier
	// implementation.
	"github.com/picfight/pfclnd/chainntnfs/bitcoindnotify"

	// Required to auto-register the pfcd backed ChainNotifier
	// implementation.
...
func testSpendNotification(miner *rpctest.Harness,
	...
	// parallel.
	//
	// Since picfightcoind is at times very slow at notifying about txs in the
...
func TestInterfaces(t *testing.T) {
	...
	for _, notifierDriver := range chainntnfs.RegisteredNotifiers() {
		...
		switch notifierType {
		case "bitcoind":
			var picfightcoindConn *chain.PicfightcoindConn
			bitcoindConn, cleanUp = chainntnfs.NewPicfightcoindBackend(
				t, p2pAddr, true,
			)
			newNotifier = func() (chainntnfs.TestChainNotifier, error) {
				return picfightcoindnotify.New(
