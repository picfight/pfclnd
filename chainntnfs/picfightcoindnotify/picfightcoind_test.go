...
// picfightcoind driver.
func setUpNotifier(t *testing.T, picfightcoindConn *chain.PicfightcoindConn,
...
func syncNotifierWithMiner(t *testing.T, notifier *PicfightcoindNotifier,
	...
	for {
		_, picfightcoindHeight, err := notifier.chainConn.GetBestBlock()
		if err != nil {
			t.Fatalf("unable to retrieve picfightcoind's current "+
				"height: %v", err)
		}

		if picfightcoindHeight == minerHeight {
...
func TestHistoricalConfDetailsTxIndex(t *testing.T) {
	...
	bitcoindConn, cleanUp := chainntnfs.NewPicfightcoindBackend(
	...
	notifier := setUpNotifier(t, picfightcoindConn, hintCache, hintCache)
	defer notifier.Stop()
