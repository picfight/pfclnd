...
type OpenChannel struct {
	...
	RemoteCurrentRevocation *pfcec.PublicKey

	// RemoteNextRevocation is the revocation key to be used for the *next*
	// commitment transaction we create for the local node. Within the
	// specification, this value is referred to as the
	// per-commitment-point.
	RemoteNextRevocation *pfcec.PublicKey
...
func fetchChanBucket(tx *bbolt.Tx, nodeKey *pfcec.PublicKey,
...
// the channel.
func (c *OpenChannel) MarkDataLoss(commitPoint *pfcec.PublicKey) error {
	c.Lock()
	defer c.Unlock()
...
func (c *OpenChannel) DataLossCommitPoint() (*pfcec.PublicKey, error) {
...
func (c *OpenChannel) InsertNextRevocation(revKey *pfcec.PublicKey) error {
	c.Lock()
	defer c.Unlock()
...
type ChannelCloseSummary struct {
	...
	TimeLockedBalance pfcutil.Amount
...
type ChannelSnapshot struct {
	...
	RemoteIdentity pfcec.PublicKey
