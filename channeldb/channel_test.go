...
var (
	...
	privKey, pubKey = pfcec.PrivKeyFromBytes(pfcec.S256(), key[:])
...
func createTestChannelState(cdb *DB) (*OpenChannel, error) {
	...
	localCfg := ChannelConfig{
		ChannelConstraints: ChannelConstraints{
			DustLimit:        pfcutil.Amount(rand.Int63()),
			MaxPendingAmount: lnwire.MilliSatoshi(rand.Int63()),
			ChanReserve:      pfcutil.Amount(rand.Int63()),
	...
	return &OpenChannel{
		...
		LocalCommitment: ChannelCommitment{
			...
			CommitFee:     pfcutil.Amount(rand.Int63()),
			FeePerKw:      pfcutil.Amount(5000),
			CommitTx:      testTx,
...
func TestOpenChannelPutGetDelete(t *testing.T) {
	...
	nextRevKey, err := pfcec.NewPrivateKey(pfcec.S256())
	if err != nil {
	...
	closeSummary := &ChannelCloseSummary{
		...
		SettledBalance:    pfcutil.Amount(500),
		TimeLockedBalance: pfcutil.Amount(10000),
		IsPending:         false,
...
func TestChannelStateTransition(t *testing.T) {
	...
	newPriv, err := pfcec.NewPrivateKey(pfcec.S256())
	if err != nil {
