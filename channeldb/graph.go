...
func (c *ChannelGraph) LookupAlias(pub *pfcec.PublicKey) (string, error) {
	var alias string
...
func (c *ChannelGraph) DeleteLightningNode(nodePub *pfcec.PublicKey) error {
...
func (l *LightningNode) PubKey() (*pfcec.PublicKey, error) {
	if l.pubKey != nil {
		return l.pubKey, nil
	}

	key, err := pfcec.ParsePubKey(l.PubKeyBytes[:], pfcec.S256())
	if err != nil {
		return nil, err
	}
	l.pubKey = key
...
func (l *LightningNode) AuthSig() (*pfcec.Signature, error) {
	return pfcec.ParseSignature(l.AuthSigBytes, pfcec.S256())
}

// AddPubKey is a setter-link method that can be used to swap out the public
// key for a node.
func (l *LightningNode) AddPubKey(key *pfcec.PublicKey) {
	l.pubKey = key
...
// returned.
func (c *ChannelGraph) FetchLightningNode(pub *pfcec.PublicKey) (*LightningNode, error) {
...
type ChannelEdgeInfo struct {
	...
	nodeKey1      *pfcec.PublicKey

	// NodeKey2Bytes is the raw public key of the first node.
	NodeKey2Bytes [33]byte
	nodeKey2      *pfcec.PublicKey

	// PicfightcoinKey1Bytes is the raw public key of the first node.
	PicfightcoinKey1Bytes [33]byte
	bitcoinKey1           *pfcec.PublicKey

	// PicfightcoinKey2Bytes is the raw public key of the first node.
	PicfightcoinKey2Bytes [33]byte
	bitcoinKey2           *pfcec.PublicKey
...
func (c *ChannelEdgeInfo) AddNodeKeys(nodeKey1, nodeKey2, picfightcoinKey1,
	bitcoinKey2 *pfcec.PublicKey) {
	...
	c.bitcoinKey1 = picfightcoinKey1
	copy(c.PicfightcoinKey1Bytes[:], c.bitcoinKey1.SerializeCompressed())

	c.bitcoinKey2 = picfightcoinKey2
	copy(c.PicfightcoinKey2Bytes[:], picfightcoinKey2.SerializeCompressed())
}
...
// NOTE: By having this method to access an attribute, we ensure we only need
// to fully deserialize the pubkey if absolutely necessary.
func (c *ChannelEdgeInfo) NodeKey1() (*pfcec.PublicKey, error) {
	if c.nodeKey1 != nil {
		return c.nodeKey1, nil
	}

	key, err := pfcec.ParsePubKey(c.NodeKey1Bytes[:], pfcec.S256())
	if err != nil {
		return nil, err
	}
	c.nodeKey1 = key
...
// NOTE: By having this method to access an attribute, we ensure we only need
// to fully deserialize the pubkey if absolutely necessary.
func (c *ChannelEdgeInfo) NodeKey2() (*pfcec.PublicKey, error) {
	if c.nodeKey2 != nil {
		return c.nodeKey2, nil
	}

	key, err := pfcec.ParsePubKey(c.NodeKey2Bytes[:], pfcec.S256())
	if err != nil {
		return nil, err
	}
	c.nodeKey2 = key
...
// PicfightcoinKey1 is the Picfightcoin multi-sig key belonging to the first
// node, that was involved in the funding transaction that originally
// created the channel that this struct represents.
//
// NOTE: By having this method to access an attribute, we ensure we only need
// to fully deserialize the pubkey if absolutely necessary.
func (c *ChannelEdgeInfo) PicfightcoinKey1() (*pfcec.PublicKey, error) {
	if c.bitcoinKey1 != nil {
		return c.bitcoinKey1, nil
	}

	key, err := pfcec.ParsePubKey(c.PicfightcoinKey1Bytes[:], pfcec.S256())
	if err != nil {
		return nil, err
	}
	c.bitcoinKey1 = key
...
// PicfightcoinKey2 is the Picfightcoin multi-sig key belonging to the second
// node, that was involved in the funding transaction that originally
// created the channel that this struct represents.
//
// NOTE: By having this method to access an attribute, we ensure we only need
// to fully deserialize the pubkey if absolutely necessary.
func (c *ChannelEdgeInfo) PicfightcoinKey2() (*pfcec.PublicKey, error) {
	if c.bitcoinKey2 != nil {
		return c.bitcoinKey2, nil
	}

	key, err := pfcec.ParsePubKey(c.PicfightcoinKey2Bytes[:], pfcec.S256())
	if err != nil {
		return nil, err
	}
	c.bitcoinKey2 = key
...
// nodeID1 || nodeID2 || picfightcoinKey1|| picfightcoinKey2 || 2-byte-feature-len ||
// features.
type ChannelAuthProof struct {
	// nodeSig1 is a cached instance of the first node signature.
	nodeSig1 *pfcec.Signature
	...
	nodeSig2 *pfcec.Signature

	// NodeSig2Bytes are the raw bytes of the second node signature
	// encoded in DER format.
	NodeSig2Bytes []byte

	// picfightcoinSig1 is a cached instance of the first picfightcoin signature.
	bitcoinSig1 *pfcec.Signature

	// PicfightcoinSig1Bytes are the raw bytes of the first picfightcoin signature
	// encoded in DER format.
	PicfightcoinSig1Bytes []byte

	// picfightcoinSig2 is a cached instance of the second picfightcoin signature.
	bitcoinSig2 *pfcec.Signature

	// PicfightcoinSig2Bytes are the raw bytes of the second picfightcoin signature
	// encoded in DER format.
	PicfightcoinSig2Bytes []byte
}
...
// NOTE: By having this method to access an attribute, we ensure we only need
// to fully deserialize the signature if absolutely necessary.
func (c *ChannelAuthProof) Node1Sig() (*pfcec.Signature, error) {
	if c.nodeSig1 != nil {
		return c.nodeSig1, nil
	}

	sig, err := pfcec.ParseSignature(c.NodeSig1Bytes, pfcec.S256())
	if err != nil {
		return nil, err
	}
...
// NOTE: By having this method to access an attribute, we ensure we only need
// to fully deserialize the signature if absolutely necessary.
func (c *ChannelAuthProof) Node2Sig() (*pfcec.Signature, error) {
	if c.nodeSig2 != nil {
		return c.nodeSig2, nil
	}

	sig, err := pfcec.ParseSignature(c.NodeSig2Bytes, pfcec.S256())
	if err != nil {
		return nil, err
	}
...
// PicfightcoinSig1 is the signature using the public key of the first node that was
// used in the channel's multi-sig output.
//
// NOTE: By having this method to access an attribute, we ensure we only need
// to fully deserialize the signature if absolutely necessary.
func (c *ChannelAuthProof) PicfightcoinSig1() (*pfcec.Signature, error) {
	if c.bitcoinSig1 != nil {
		return c.bitcoinSig1, nil
	}

	sig, err := pfcec.ParseSignature(c.PicfightcoinSig1Bytes, pfcec.S256())
	if err != nil {
		return nil, err
	}
...
// PicfightcoinSig2 is the signature using the public key of the second node that
// was used in the channel's multi-sig output.
//
// NOTE: By having this method to access an attribute, we ensure we only need
// to fully deserialize the signature if absolutely necessary.
func (c *ChannelAuthProof) PicfightcoinSig2() (*pfcec.Signature, error) {
	if c.bitcoinSig2 != nil {
		return c.bitcoinSig2, nil
	}

	sig, err := pfcec.ParseSignature(c.PicfightcoinSig2Bytes, pfcec.S256())
	if err != nil {
		return nil, err
	}
...
func (c *ChannelAuthProof) IsEmpty() bool {
	return len(c.NodeSig1Bytes) == 0 ||
		...
		len(c.PicfightcoinSig1Bytes) == 0 ||
		len(c.PicfightcoinSig2Bytes) == 0
}
...
// NOTE: By having this method to access an attribute, we ensure we only need
// to fully deserialize the signature if absolutely necessary.
func (c *ChannelEdgePolicy) Signature() (*pfcec.Signature, error) {
	if c.sig != nil {
		return c.sig, nil
	}

	sig, err := pfcec.ParseSignature(c.SigBytes, pfcec.S256())
	if err != nil {
		return nil, err
	}
	...
	if err := c.db.View(func(tx *bbolt.Tx) error {
		...
		return chanIndex.ForEach(func(chanPointBytes, chanID []byte) error {
			...
			pkScript, err := genMultiSigP2WSH(
				edgeInfo.PicfightcoinKey1Bytes[:],
				edgeInfo.PicfightcoinKey2Bytes[:],
			)
			if err != nil {
				return err
			}
...
func putChanEdgeInfo(edgeIndex *bbolt.Bucket, edgeInfo *ChannelEdgeInfo, chanID [8]byte) error {
	...
	if _, err := b.Write(edgeInfo.PicfightcoinKey1Bytes[:]); err != nil {
		return err
	}
	if _, err := b.Write(edgeInfo.PicfightcoinKey2Bytes[:]); err != nil {
		return err
	}
	...
	var nodeSig1, nodeSig2, picfightcoinSig1, picfightcoinSig2 []byte
	if authProof != nil {
		nodeSig1 = authProof.NodeSig1Bytes
		nodeSig2 = authProof.NodeSig2Bytes
		bitcoinSig1 = authProof.PicfightcoinSig1Bytes
		bitcoinSig2 = authProof.PicfightcoinSig2Bytes
	}
	...
	if err := wire.WriteVarBytes(&b, 0, picfightcoinSig1); err != nil {
		return err
	}
	if err := wire.WriteVarBytes(&b, 0, picfightcoinSig2); err != nil {
		return err
	}
...
func deserializeChanEdgeInfo(r io.Reader) (ChannelEdgeInfo, error) {
	...
	if _, err := io.ReadFull(r, edgeInfo.PicfightcoinKey1Bytes[:]); err != nil {
		return ChannelEdgeInfo{}, err
	}
	if _, err := io.ReadFull(r, edgeInfo.PicfightcoinKey2Bytes[:]); err != nil {
		return ChannelEdgeInfo{}, err
	}
	...
	proof.PicfightcoinSig1Bytes, err = wire.ReadVarBytes(r, 0, 80, "sigs")
	if err != nil {
		return ChannelEdgeInfo{}, err
	}
	proof.PicfightcoinSig2Bytes, err = wire.ReadVarBytes(r, 0, 80, "sigs")
	if err != nil {
		return ChannelEdgeInfo{}, err
	}
