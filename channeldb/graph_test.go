...
func createTestVertex(db *DB) (*LightningNode, error) {
	...
	priv, err := pfcec.NewPrivateKey(pfcec.S256())
	if err != nil {
		return nil, err
	}
...
func TestNodeInsertionAndDeletion(t *testing.T) {
	...
	_, testPub := pfcec.PrivKeyFromBytes(pfcec.S256(), key[:])
	node := &LightningNode{
		HaveNodeAnnouncement: true,
		AuthSigBytes:         testSig.Serialize(),
...
func TestEdgeInsertionDeletion(t *testing.T) {
	...
	edgeInfo := ChannelEdgeInfo{
		...
		AuthProof: &ChannelAuthProof{
			...
			PicfightcoinSig1Bytes: testSig.Serialize(),
			PicfightcoinSig2Bytes: testSig.Serialize(),
		},
	...
	copy(edgeInfo.PicfightcoinKey1Bytes[:], node1Pub.SerializeCompressed())
	copy(edgeInfo.PicfightcoinKey2Bytes[:], node2Pub.SerializeCompressed())
...
func assertEdgeInfoEqual(t *testing.T, e1 *ChannelEdgeInfo,
	...
	if !bytes.Equal(e1.PicfightcoinKey1Bytes[:], e2.PicfightcoinKey1Bytes[:]) {
		t.Fatalf("bitcoinkey1 doesn't match")
	}
	if !bytes.Equal(e1.PicfightcoinKey2Bytes[:], e2.PicfightcoinKey2Bytes[:]) {
	...
	if !bytes.Equal(e1.AuthProof.PicfightcoinSig1Bytes, e2.AuthProof.PicfightcoinSig1Bytes) {
		t.Fatalf("bitcoinsig1 doesn't match")
	}
	if !bytes.Equal(e1.AuthProof.PicfightcoinSig2Bytes, e2.AuthProof.PicfightcoinSig2Bytes) {
...
func TestEdgeInfoUpdates(t *testing.T) {
	...
	copy(edgeInfo.PicfightcoinKey1Bytes[:], firstNode.PubKeyBytes[:])
	copy(edgeInfo.PicfightcoinKey2Bytes[:], secondNode.PubKeyBytes[:])
	if err := graph.AddChannelEdge(edgeInfo); err != nil {
		t.Fatalf("unable to create channel edge: %v", err)
	}
...
func TestGraphTraversal(t *testing.T) {
	...
	for i := 0; i < numChannels; i++ {
		...
		copy(edgeInfo.PicfightcoinKey1Bytes[:], nodes[0].PubKeyBytes[:])
		copy(edgeInfo.PicfightcoinKey2Bytes[:], nodes[1].PubKeyBytes[:])
...
func TestGraphPruning(t *testing.T) {
	...
	for i := 0; i < numNodes-1; i++ {
		...
		copy(edgeInfo.PicfightcoinKey1Bytes[:], graphNodes[i].PubKeyBytes[:])
		copy(edgeInfo.PicfightcoinKey2Bytes[:], graphNodes[i+1].PubKeyBytes[:])
		if err := graph.AddChannelEdge(&edgeInfo); err != nil {
			t.Fatalf("unable to add node: %v", err)
		}

		pkScript, err := genMultiSigP2WSH(
			edgeInfo.PicfightcoinKey1Bytes[:], edgeInfo.PicfightcoinKey2Bytes[:],
		)
		if err != nil {
