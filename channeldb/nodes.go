...
// channel open with. Information such as the Picfightcoin network the node
...
// Picfightcoin's addrmanager. The TCP address information stored within the struct
...
type LinkNode struct {
	// Network indicates the Picfightcoin network that the LinkNode advertises
...
func (db *DB) NewLinkNode(bitNet wire.PicfightcoinNet, pub *pfcec.PublicKey,
...
// database.
func (db *DB) DeleteLinkNode(identity *pfcec.PublicKey) error {
	return db.Update(func(tx *bbolt.Tx) error {
		return db.deleteLinkNode(tx, identity)
	})
}

func (db *DB) deleteLinkNode(tx *bbolt.Tx, identity *pfcec.PublicKey) error {
	nodeMetaBucket := tx.Bucket(nodeInfoBucket)
	if nodeMetaBucket == nil {
		return ErrLinkNodesNotFound
	}
...
func (db *DB) FetchLinkNode(identity *pfcec.PublicKey) (*LinkNode, error) {
	var (
		node *LinkNode
		err  error
	)
...
func deserializeLinkNode(r io.Reader) (*LinkNode, error) {
	...
	node.Network = wire.PicfightcoinNet(byteOrder.Uint32(buf[:4]))

	var pub [33]byte
	if _, err := io.ReadFull(r, pub[:]); err != nil {
		return nil, err
	}
	node.IdentityPub, err = pfcec.ParsePubKey(pub[:], pfcec.S256())
	if err != nil {
		return nil, err
	}
