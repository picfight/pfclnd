...
import (
	...
	"github.com/picfight/pfclnd/build"
	"github.com/picfight/pfclnd/htlcswitch/hodl"
	"github.com/picfight/pfclnd/lncfg"
	"github.com/picfight/pfclnd/lnrpc/signrpc"
	"github.com/picfight/pfclnd/lnwire"
	"github.com/picfight/pfclnd/routing"
	"github.com/picfight/pfclnd/tor"
...
var (
	defaultLndDir     = pfcutil.AppDataDir("lnd", false)
	...
	defaultPfcdDir         = pfcutil.AppDataDir("pfcd", false)
	defaultPfcdRPCCertFile = filepath.Join(defaultPfcdDir, "rpc.cert")

	defaultLtcdDir         = pfcutil.AppDataDir("ltcd", false)
	defaultLtcdRPCCertFile = filepath.Join(defaultLtcdDir, "rpc.cert")

	defaultPicfightcoindDir = pfcutil.AppDataDir("bitcoin", false)
	defaultLitecoindDir     = pfcutil.AppDataDir("litecoin", false)
...
type chainConfig struct {
	...
	Node string `long:"node" description:"The blockchain interface to use." choice:"pfcd" choice:"bitcoind" choice:"neutrino" choice:"ltcd" choice:"litecoind"`
...
type picfightcoindConfig struct {
	Dir            string `long:"dir" description:"The base directory that contains the node's data, logs, configuration file, etc."`
	RPCHost        string `long:"rpchost" description:"The daemon's rpc listening address. If a port is omitted, then the default port for the selected chain parameters will be used."`
	RPCUser        string `long:"rpcuser" description:"Username for RPC connections"`
	RPCPass        string `long:"rpcpass" default-mask:"-" description:"Password for RPC connections"`
...
type config struct {
	...
	Picfightcoin      *chainConfig    `group:"Picfightcoin" namespace:"bitcoin"`
	PfcdMode          *pfcdConfig     `group:"pfcd" namespace:"pfcd"`
	PicfightcoindMode *bitcoindConfig `group:"bitcoind" namespace:"bitcoind"`
	NeutrinoMode      *neutrinoConfig `group:"neutrino" namespace:"neutrino"`

	Litecoin      *chainConfig    `group:"Litecoin" namespace:"litecoin"`
	LtcdMode      *pfcdConfig     `group:"ltcd" namespace:"ltcd"`
...
func loadConfig() (*config, error) {
	defaultCfg := config{
		...
		Picfightcoin: &chainConfig{
			MinHTLC:       defaultPicfightcoinMinHTLCMSat,
			BaseFee:       defaultPicfightcoinBaseFeeMSat,
			FeeRate:       defaultPicfightcoinFeeRate,
			TimeLockDelta: defaultPicfightcoinTimeLockDelta,
			Node:          "pfcd",
		},
		PfcdMode: &pfcdConfig{
			Dir:     defaultPfcdDir,
			RPCHost: defaultRPCHost,
			RPCCert: defaultPfcdRPCCertFile,
		},
		PicfightcoindMode: &bitcoindConfig{
			Dir:     defaultPicfightcoindDir,
			RPCHost: defaultRPCHost,
		},
	...
	cfg.PfcdMode.Dir = cleanAndExpandPath(cfg.PfcdMode.Dir)
	cfg.LtcdMode.Dir = cleanAndExpandPath(cfg.LtcdMode.Dir)
	cfg.PicfightcoindMode.Dir = cleanAndExpandPath(cfg.PicfightcoindMode.Dir)
	...
	case cfg.Litecoin.Active && cfg.Picfightcoin.Active:
		str := "%s: Currently both Picfightcoin and Litecoin cannot be " +
			"active together"
		return nil, fmt.Errorf(str, funcName)

	// Either Picfightcoin must be active, or Litecoin must be active.
	// Otherwise, we don't know which chain we're on.
	case !cfg.Picfightcoin.Active && !cfg.Litecoin.Active:
		return nil, fmt.Errorf("%s: either picfightcoin.active or "+
	...
	case cfg.Litecoin.Active:
		...
		// picfightcoin with the litecoin specific information.
	...
	case cfg.Picfightcoin.Active:
		...
		// while we're at it.
		numNets := 0
		if cfg.Picfightcoin.MainNet {
			numNets++
			activeNetParams = picfightcoinMainNetParams
		}
		if cfg.Picfightcoin.TestNet3 {
			numNets++
			activeNetParams = picfightcoinTestNetParams
		}
		if cfg.Picfightcoin.RegTest {
			numNets++
			activeNetParams = regTestNetParams
		}
		if cfg.Picfightcoin.SimNet {
			numNets++
			activeNetParams = picfightcoinSimNetParams
		}
		if numNets > 1 {
		...
		if numNets == 0 {
			str := "%s: either --bitcoin.mainnet, or " +
				"bitcoin.testnet, picfightcoin.simnet, or picfightcoin.regtest " +
				"must be specified"
			err := fmt.Errorf(str, funcName)
			return nil, err
		}

		if cfg.Picfightcoin.MainNet && cfg.DebugHTLC {
			str := "%s: debug-htlc mode cannot be used " +
				"on picfightcoin mainnet"
			err := fmt.Errorf(str, funcName)
			return nil, err
		}

		if cfg.Picfightcoin.Node == "neutrino" && cfg.Picfightcoin.MainNet {
		...
		if cfg.Picfightcoin.TimeLockDelta < minTimeLockDelta {
			return nil, fmt.Errorf("timelockdelta must be at least %v",
				minTimeLockDelta)
		}

		switch cfg.Picfightcoin.Node {
		case "pfcd":
			err := parseRPCParams(
				cfg.Picfightcoin, cfg.PfcdMode, picfightcoinChain, funcName,
			)
			if err != nil {
				err := fmt.Errorf("unable to load RPC "+
					"credentials for pfcd: %v", err)
				return nil, err
			}
		case "bitcoind":
			if cfg.Picfightcoin.SimNet {
				return nil, fmt.Errorf("%s: picfightcoind does not "+
					"support simnet", funcName)
			}

			err := parseRPCParams(
				cfg.Picfightcoin, cfg.PicfightcoindMode, picfightcoinChain, funcName,
			)
			if err != nil {
				err := fmt.Errorf("unable to load RPC "+
					"credentials for picfightcoind: %v", err)
				return nil, err
			}
		case "neutrino":
		...
		default:
			str := "%s: only pfcd, picfightcoind, and neutrino mode " +
				"supported for picfightcoin at this time"
			return nil, fmt.Errorf(str, funcName)
		}

		cfg.Picfightcoin.ChainDir = filepath.Join(cfg.DataDir,
			defaultChainSubDirname,
			bitcoinChain.String())

		// Finally we'll register the picfightcoin chain as our current
		// primary chain.
...
// This function is taken from https://github.com/picfight/pfcd
...
func parseRPCParams(cConfig *chainConfig, nodeConfig interface{}, net chainCode,
	...
	// If we're in simnet mode, then the running pfcd instance won't read
	// the RPC credentials from the configuration. So if lnd wasn't
	// specified the parameters, then we won't be able to start.
	if cConfig.SimNet {
		str := "%v: rpcuser and rpcpass must be set to your pfcd " +
	...
	switch cConfig.Node {
	case "pfcd", "ltcd":
		nConf := nodeConfig.(*pfcdConfig)
		rpcUser, rpcPass, err := extractPfcdRPCParams(confFile)
		if err != nil {
	...
	case "bitcoind", "litecoind":
		...
		rpcUser, rpcPass, zmqBlockHost, zmqTxHost, err :=
			extractPicfightcoindRPCParams(confFile)
		if err != nil {
			return fmt.Errorf("unable to extract RPC credentials:"+
				" %v, cannot start w/o RPC connection",
				err)
...
// extractPfcdRPCParams attempts to extract the RPC credentials for an existing
// pfcd instance. The passed path is expected to be the location of pfcd's
// application data directory on the target system.
func extractPfcdRPCParams(pfcdConfigPath string) (string, string, error) {
	// First, we'll open up the pfcd configuration file found at the target
	// destination.
	...
	configContents, err := ioutil.ReadAll(pfcdConfigFile)
	if err != nil {
		return "", "", err
	}
...
// extractPicfightcoindParams attempts to extract the RPC credentials for an
// existing picfightcoind node instance. The passed path is expected to be the
// location of picfightcoind's picfightcoin.conf on the target system. The routine looks
// for a cookie first, optionally following the datadir configuration option in
// the picfightcoin.conf. If it doesn't find one, it looks for rpcuser/rpcpassword.
func extractPicfightcoindRPCParams(bitcoindConfigPath string) (string, string, string, string, error) {
	// First, we'll open up the picfightcoind configuration file found at the
	// target destination.
	bitcoindConfigFile, err := os.Open(bitcoindConfigPath)
	if err != nil {
		return "", "", "", "", err
	}
	defer picfightcoindConfigFile.Close()
