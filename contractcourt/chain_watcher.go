...
type chainWatcherConfig struct {
	...
	// known to us.
	isOurAddr func(pfcutil.Address) bool
}
...
// being dust.
func (c *chainWatcher) toSelfAmount(tx *wire.MsgTx) pfcutil.Amount {
	var selfAmt pfcutil.Amount
	for _, txOut := range tx.TxOut {
		...
		for _, addr := range addrs {
			if c.cfg.isOurAddr(addr) {
				selfAmt += pfcutil.Amount(txOut.Value)
			}
		}
	}
...
func (c *chainWatcher) dispatchLocalForceClose(
	...
	for _, htlc := range forceClose.HtlcResolutions.OutgoingHTLCs {
		htlcValue := pfcutil.Amount(htlc.SweepSignDesc.Output.Value)
...
func (c *chainWatcher) dispatchRemoteForceClose(
	...
	commitPoint *pfcec.PublicKey) error {
