...
import (
	...
	"github.com/picfight/pfclnd/multimutex"
...
type Config struct {
	...
	FindPeer func(identityKey *pfcec.PublicKey) (lnpeer.Peer, error)
...
func New(cfg Config, selfKey *pfcec.PublicKey) (*AuthenticatedGossiper, error) {
...
func (d *AuthenticatedGossiper) ProcessLocalAnnouncement(msg lnwire.Message,
	source *pfcec.PublicKey) chan error {
...
func (d *AuthenticatedGossiper) resendAnnounceSignatures() error {
	...
	if err := d.cfg.DB.View(func(tx *bbolt.Tx) error {
		...
		if err := bucket.ForEach(func(k, v []byte) error {
			...
			// public key.
			peer, err := pfcec.ParsePubKey(k[:33], pfcec.S256())
			if err != nil {
				return err
			}
...
// for the target peer.
func (d *AuthenticatedGossiper) findGossipSyncer(pub *pfcec.PublicKey) (
	*gossipSyncer, error) {
...
func (d *AuthenticatedGossiper) PruneSyncState(peer *pfcec.PublicKey) {
	d.syncerMtx.Lock()
	defer d.syncerMtx.Unlock()
...
func (d *AuthenticatedGossiper) processNetworkAnnouncement(
	...
	case *lnwire.ChannelAnnouncement:
		...
		if nMsg.isRemote {
			...
			proof = &channeldb.ChannelAuthProof{
				...
				PicfightcoinSig1Bytes: msg.PicfightcoinSig1.ToSignatureBytes(),
				PicfightcoinSig2Bytes: msg.PicfightcoinSig2.ToSignatureBytes(),
			}
		}
		...
		edge := &channeldb.ChannelEdgeInfo{
			...
			PicfightcoinKey1Bytes: msg.PicfightcoinKey1,
			PicfightcoinKey2Bytes: msg.PicfightcoinKey2,
			AuthProof:             proof,
	...
	case *lnwire.AnnounceSignatures:
		...
		// registered in picfightcoin blockchain. Therefore, we check if the
		...
		// announcement.
		if !nMsg.isRemote {
			var remotePeer *pfcec.PublicKey
			if isFirstNode {
		...
		if isFirstNode {
			...
			dbProof.PicfightcoinSig1Bytes = msg.PicfightcoinSignature.ToSignatureBytes()
			dbProof.PicfightcoinSig2Bytes = oppositeProof.PicfightcoinSignature.ToSignatureBytes()
		} else {
			...
			dbProof.PicfightcoinSig1Bytes = oppositeProof.PicfightcoinSignature.ToSignatureBytes()
			dbProof.PicfightcoinSig2Bytes = msg.PicfightcoinSignature.ToSignatureBytes()
		}
		chanAnn, e1Ann, e2Ann, err := CreateChanAnnouncement(
		...
		// existence of funding point and inclusion of nodes picfightcoin
		// keys in it already checked by the router. In this stage we
		// should check that node keys are attest to the picfightcoin keys
...
func (d *AuthenticatedGossiper) sendAnnSigReliably(
	msg *lnwire.AnnounceSignatures, remotePeer *pfcec.PublicKey) error {
	...
	*lnwire.ChannelUpdate, error) {
	...
	if info.AuthProof != nil {
		...
		chanAnn = &lnwire.ChannelAnnouncement{
			...
			ChainHash:        info.ChainHash,
			PicfightcoinKey1: info.PicfightcoinKey1Bytes,
			Features:         lnwire.NewRawFeatureVector(),
			PicfightcoinKey2: info.PicfightcoinKey2Bytes,
			ExtraOpaqueData:  edge.ExtraOpaqueData,
		}
		...
		chanAnn.PicfightcoinSig1, err = lnwire.NewSigFromRawSignature(
			info.AuthProof.PicfightcoinSig1Bytes,
		)
		if err != nil {
			return nil, nil, err
		}
		chanAnn.PicfightcoinSig2, err = lnwire.NewSigFromRawSignature(
			info.AuthProof.PicfightcoinSig2Bytes,
		)
		if err != nil {
			return nil, nil, err
		}
