...
var (
	...
	bitcoinKeyPriv1, _ = pfcec.NewPrivateKey(pfcec.S256())
	bitcoinKeyPub1     = picfightcoinKeyPriv1.PubKey()

	nodeKeyPriv1, _ = pfcec.NewPrivateKey(pfcec.S256())
	nodeKeyPub1     = nodeKeyPriv1.PubKey()

	bitcoinKeyPriv2, _ = pfcec.NewPrivateKey(pfcec.S256())
	bitcoinKeyPub2     = picfightcoinKeyPriv2.PubKey()

	nodeKeyPriv2, _ = pfcec.NewPrivateKey(pfcec.S256())
...
func (n *mockSigner) SignMessage(pubKey *pfcec.PublicKey,
...
func createAnnouncements(blockHeight uint32) (*annBatch, error) {
	...
	batch.localProofAnn = &lnwire.AnnounceSignatures{
		...
		PicfightcoinSignature: batch.remoteChanAnn.PicfightcoinSig1,
	}

	batch.remoteProofAnn = &lnwire.AnnounceSignatures{
		NodeSignature:         batch.remoteChanAnn.NodeSig2,
		PicfightcoinSignature: batch.remoteChanAnn.PicfightcoinSig2,
	}
...
func createNodeAnnouncement(priv *pfcec.PrivateKey,
...
func createUpdateAnnouncement(blockHeight uint32, flags lnwire.ChanUpdateFlag,
	nodeKey *pfcec.PrivateKey, timestamp uint32,
...
func createAnnouncementWithoutProof(blockHeight uint32,
	...
	copy(a.PicfightcoinKey1[:], picfightcoinKeyPub1.SerializeCompressed())
	copy(a.PicfightcoinKey2[:], picfightcoinKeyPub2.SerializeCompressed())
	if len(extraBytes) == 1 {
		a.ExtraOpaqueData = extraBytes[0]
	}
...
func createRemoteChannelAnnouncement(blockHeight uint32,
	...
	pub = picfightcoinKeyPriv1.PubKey()
	signer = mockSigner{bitcoinKeyPriv1}
	sig, err = SignAnnouncement(&signer, pub, a)
	if err != nil {
		return nil, err
	}
	a.PicfightcoinSig1, err = lnwire.NewSigFromSignature(sig)
	if err != nil {
		return nil, err
	}

	pub = picfightcoinKeyPriv2.PubKey()
	signer = mockSigner{bitcoinKeyPriv2}
	sig, err = SignAnnouncement(&signer, pub, a)
	if err != nil {
		return nil, err
	}
	a.PicfightcoinSig2, err = lnwire.NewSigFromSignature(sig)
	if err != nil {
		return nil, err
	}
...
func createTestCtx(startHeight uint32) (*testCtx, func(), error) {
	...
	gossiper, err := New(Config{
		...
		SendToPeer: func(target *pfcec.PublicKey, msg ...lnwire.Message) error {
			return nil
		},
		FindPeer: func(target *pfcec.PublicKey) (lnpeer.Peer, error) {
...
func TestProcessAnnouncement(t *testing.T) {
	...
	assertSenderExistence := func(sender *pfcec.PublicKey, msg msgWithSenders) {
...
func TestSignatureAnnouncementLocalFirst(t *testing.T) {
	...
	ctx.gossiper.cfg.FindPeer = func(target *pfcec.PublicKey) (lnpeer.Peer, error) {
		return &mockPeer{target, sentMsgs, ctx.gossiper.quit}, nil
	}
	ctx.gossiper.cfg.SendToPeer = func(target *pfcec.PublicKey,
	...
	localKey, err := pfcec.ParsePubKey(batch.nodeAnn1.NodeID[:], pfcec.S256())
	if err != nil {
		t.Fatalf("unable to parse pubkey: %v", err)
	}
	remoteKey, err := pfcec.ParsePubKey(batch.nodeAnn2.NodeID[:], pfcec.S256())
	if err != nil {
		t.Fatalf("unable to parse pubkey: %v", err)
	}
...
func TestOrphanSignatureAnnouncement(t *testing.T) {
	...
	ctx.gossiper.cfg.SendToPeer = func(target *pfcec.PublicKey, msg ...lnwire.Message) error {
		select {
		case sentMsgs <- msg[0]:
		case <-ctx.gossiper.quit:
			return fmt.Errorf("shutting down")
...
func TestSignatureAnnouncementRetry(t *testing.T) {
	...
	ctx.gossiper.cfg.NotifyWhenOnline = func(peer *pfcec.PublicKey,
...
func TestSignatureAnnouncementRetryAtStartup(t *testing.T) {
	...
	gossiper, err := New(Config{
		...
		SendToPeer: func(target *pfcec.PublicKey,
			msg ...lnwire.Message) error {
			return fmt.Errorf("intentional error in SendToPeer")
		},
		NotifyWhenOnline: func(peer *pfcec.PublicKey,
			connectedChan chan<- lnpeer.Peer) {
			notifyPeers <- connectedChan
		},
...
func TestNodeAnnouncementNoChannels(t *testing.T) {
	...
	remoteKey, err := pfcec.ParsePubKey(batch.nodeAnn2.NodeID[:],
		pfcec.S256())
	if err != nil {
		t.Fatalf("unable to parse pubkey: %v", err)
	}
...
func (p *mockPeer) IdentityKey() *pfcec.PublicKey      { return p.pk }
