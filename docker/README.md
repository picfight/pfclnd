...
package `lnd` and `pfcd` together to make deploying the two daemons as easy as
typing a few commands. All configuration between `lnd` and `pfcd` are handled
...
that's similar to Picfightcoin Core's `regtest` mode. In `simnet` mode blocks can be
...
next versions will use the started `pfcd` picfightcoin node in `testnet` and
`faucet` wallet from which you will get the picfightcoins.
...
+ ----- +                   + --- +         create channels and interact with each other using the   
    |                          |            Picfightcoin network as source of truth. 
    |                          |            
    + - - - -  - + - - - - - - +            
                 |
        + --------------- +
        | Picfightcoin network |  <---  In the current scenario for simplicity we create only one  
        + --------------- +        "pfcd" node which represents the Picfightcoin network, in a 
                                    real situation Alice and Bob will likely be 
                                    connected to different Picfightcoin nodes.
```

**General workflow is the following:** 

 * Create a `pfcd` node running on a private `simnet`.
 * Create `Alice`, one of the `lnd` nodes in our simulation network.
 * Create `Bob`, the other `lnd` node in our simulation network.
 * Mine some blocks to send `Alice` some picfightcoins.
 * Open channel between `Alice` and `Bob`.
 * Send payment from `Alice` to `Bob`.
 * Close the channel between `Alice` and `Bob`.
 * Check that on-chain `Bob` balance was changed.

Start `pfcd`, and then create an address for `Alice` that we'll directly mine
bitcoin into.
```bash
# Init picfightcoin network env variable:
...
# Recreate "pfcd" node and set Alice's address as mining address:
$ MINING_ADDRESS=<alice_address> docker-compose up -d pfcd
...
+ ----- +                   + ------ +                    + --- +        
 ...
 create channels to interact with each other using the Picfightcoin network 
 as source of truth.
 
 (3) In current scenario "Alice" and "Faucet" lightning network nodes 
 connect to different Picfightcoin nodes. If you decide to connect "Bob"
 to "Faucet" then the already created "pfcd" node would be sufficient.
```

First of all you need to run `pfcd` node in `testnet` and wait for it to be 
...
# Run "pfcd" node:
$ docker-compose up -d "pfcd"
```

After `pfcd` synced, connect `Alice` to the `Faucet` node.
...
and send some amount of picfightcoins to `Alice`.
...
* How to see `alice` | `bob` | `pfcd` logs?
```bash
docker-compose logs <alice|bob|pfcd>
```
