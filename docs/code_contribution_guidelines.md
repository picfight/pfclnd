...
- [Lightning Network Whitepaper](https://lightning.network/lightning-network-paper.pdf) - This is the white paper that kicked off the Layer 2 revolution. Having a good grasp of the concepts of Lightning will make the core logic within the daemon much more comprehensible: Picfightcoin Script, off-chain blockchain protocols, payment channels, bi-directional payment channels, relative and absolute time-locks, commitment state revocations, and Segregated Witness. 
    - The original LN was written for a rather narrow audience, the paper may be a bit unapproachable to many. Thanks to the Picfightcoin community, there exist many easily accessible supplemental resources which can help one see how all the pieces fit together from double-spend protection all the way up to commitment state transitions and Hash Time Locked Contracts (HTLCs): 
...
- Changes to publicly exported packages such as
  [brontide](https://github.com/picfight/pfclnd/tree/master/brontide) should
...
- Changes to behavior within the daemon's interaction with the P2P protocol,
  ...
  [`networkHarness`framework](https://github.com/picfight/pfclnd/blob/master/lntest/harness.go)
  contained within `lnd`. For example integration tests, see
  [`lnd_test.go`](https://github.com/picfight/pfclnd/blob/master/lnd_test.go#L181). 
...
[perusing the make file documentation](https://github.com/picfight/pfclnd/blob/master/docs/MAKEFILE.md).
...
func DeriveRevocationPubkey(commitPubKey *pfcec.PublicKey,
	revokePreimage []byte) *pfcec.PublicKey {
```
**RIGHT**
```go
...
definitions, check out the [lnrpc README](https://github.com/picfight/pfclnd/blob/master/lnrpc/README.md).
...
Rejoice as you will now be listed as a [contributor](https://github.com/picfight/pfclnd/graphs/contributors)!
...
[MIT license](https://github.com/picfight/pfclnd/blob/master/LICENSE).  This is
...
contribution](https://github.com/picfight/pfcd/blob/master/docs/code_contribution_guidelines.md)
guidelines for pfcd. 
