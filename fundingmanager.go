...
import (
	...
	"github.com/picfight/pfclnd/lnpeer"
...
const (
	...
	// Picfightcoin CSV delay we will require the remote to use for its
	...
	minChanFundingSize = pfcutil.Amount(20000)

	// maxBtcFundingAmount is a soft-limit of the maximum channel size
	// currently accepted on the Picfightcoin chain within the Lightning
	// Protocol. This limit is defined in BOLT-0002, and serves as an
	// initial precautionary limit while implementations are battle tested
	// in the real world.
	maxBtcFundingAmount = pfcutil.Amount(1<<24) - 1
...
var (
	...
	// to the value under the Picfightcoin chain as default.
	//
...
// live pubkey object.
func newSerializedKey(pubKey *pfcec.PublicKey) serializedPubKey {
...
type fundingConfig struct {
	...
	// distinct sub-system?
	SignMessage func(pubKey *pfcec.PublicKey, msg []byte) (*pfcec.Signature, error)
	...
	NotifyWhenOnline func(peer *pfcec.PublicKey, peerChan chan<- lnpeer.Peer)
	...
	NumRequiredConfs func(pfcutil.Amount, lnwire.MilliSatoshi) uint16
	...
	// contract breach.
	RequiredRemoteDelay func(pfcutil.Amount) uint16

	// RequiredRemoteChanReserve is a function closure that, given the
	// channel capacity and dust limit, will return an appropriate amount
	// for the remote peer's required channel reserve that is to be adhered
	// to at all times.
	RequiredRemoteChanReserve func(capacity, dustLimit pfcutil.Amount) pfcutil.Amount

	// RequiredRemoteMaxValue is a function closure that, given the channel
	// capacity, returns the amount of MilliSatoshis that our remote peer
	// can have in total outstanding HTLCs with us.
	RequiredRemoteMaxValue func(pfcutil.Amount) lnwire.MilliSatoshi

	// RequiredRemoteMaxHTLCs is a function closure that, given the channel
	// capacity, returns the number of maximum HTLCs the remote peer can
	// offer us.
	RequiredRemoteMaxHTLCs func(pfcutil.Amount) uint16
	...
	WatchNewChannel func(*channeldb.OpenChannel, *pfcec.PublicKey) error
...
type pendingChannel struct {
	identityPub   *pfcec.PublicKey
	channelPoint  *wire.OutPoint
	capacity      pfcutil.Amount
	localBalance  pfcutil.Amount
	remoteBalance pfcutil.Amount
}
...
func (f *fundingManager) handleFundingOpen(fmsg *fundingOpenMsg) {
	...
	if amt < f.cfg.MinChanSize {
		f.failFundingFlow(
			fmsg.peer, fmsg.msg.PendingChannelID,
			lnwallet.ErrChanTooSmall(amt, pfcutil.Amount(f.cfg.MinChanSize)),
		)
		return
	}
...
type channelProof struct {
	nodeSig    *pfcec.Signature
	bitcoinSig *pfcec.Signature
}
...
func (f *fundingManager) newChanAnnouncement(localPubKey, remotePubKey,
	localFundingKey, remoteFundingKey *pfcec.PublicKey,
	...
	if bytes.Compare(selfBytes, remoteBytes) == -1 {
		...
		copy(chanAnn.PicfightcoinKey1[:], localFundingKey.SerializeCompressed())
		copy(chanAnn.PicfightcoinKey2[:], remoteFundingKey.SerializeCompressed())
		...
		copy(chanAnn.PicfightcoinKey1[:], remoteFundingKey.SerializeCompressed())
		copy(chanAnn.PicfightcoinKey2[:], localFundingKey.SerializeCompressed())
	...
	if err != nil {
		return nil, errors.Errorf("unable to generate picfightcoin "+
	...
	proof.PicfightcoinSignature, err = lnwire.NewSigFromSignature(bitcoinSig)
	if err != nil {
		return nil, err
	}
...
func (f *fundingManager) announceChannel(localIDKey, remoteIDKey, localFundingKey,
	remoteFundingKey *pfcec.PublicKey, shortChanID lnwire.ShortChannelID,
...
func (f *fundingManager) handleInitFundingMsg(msg *initFundingMsg) {
	...
	var ourDustLimit pfcutil.Amount
...
// reservation.
func (f *fundingManager) cancelReservationCtx(peerKey *pfcec.PublicKey,
...
func (f *fundingManager) deleteReservationCtx(peerKey *pfcec.PublicKey,
...
func (f *fundingManager) getReservationCtx(peerKey *pfcec.PublicKey,
	pendingChanID [32]byte) (*reservationWithCtx, error) {
