...
func init() {
	...
	var err error
	sphinxPrivKey, err = pfcec.NewPrivateKey(pfcec.S256())
	if err != nil {
		panic(err)
	}

	// And another, whose public key will serve as the test ephemeral key.
	testEphemeralPriv, err := pfcec.NewPrivateKey(pfcec.S256())
	if err != nil {
		panic(err)
	}
...
var halfCircuitTests = []struct {
	hash      [32]byte
	inValue   pfcutil.Amount
	outValue  pfcutil.Amount
