...
import (
	...
	"github.com/picfight/pfclnd/contractcourt"
	...
	"github.com/picfight/pfclnd/ticker"
...
func TestChannelLinkSingleHopPayment(t *testing.T) {
	...
	channels, cleanUp, _, err := createClusterChannels(
		pfcutil.SatoshiPerPicfightcoin*3,
		pfcutil.SatoshiPerPicfightcoin*5)
	if err != nil {
	...
	amount := lnwire.NewMSatFromSatoshis(pfcutil.SatoshiPerPicfightcoin)
...
func TestExitNodeTimelockPayloadMismatch(t *testing.T) {
	...
	const amount = pfcutil.SatoshiPerPicfightcoin
...
func TestLinkForwardTimelockPolicyMismatch(t *testing.T) {
	...
	// We'll be sending 1 PFC over a 2-hop (3 vertex) route.
	amount := lnwire.NewMSatFromSatoshis(pfcutil.SatoshiPerPicfightcoin)
...
func TestLinkForwardFeePolicyMismatch(t *testing.T) {
	...
	// We'll be sending 1 PFC over a 2-hop (3 vertex) route. Given the
	// current default fee of 1 SAT, if we just send a single PFC over in
	// an HTLC, it should be rejected.
	amountNoFee := lnwire.NewMSatFromSatoshis(pfcutil.SatoshiPerPicfightcoin)
...
// bob<->alice channel has insufficient PFC capacity/bandwidth. In this test we
...
func TestChannelLinkMultiHopInsufficientPayment(t *testing.T) {
	...
	// We'll attempt to send 4 PFC although the alice-to-bob channel only
	// has 3 PFC total capacity. As a result, this payment should be
	// rejected.
	amount := lnwire.NewMSatFromSatoshis(4 * pfcutil.SatoshiPerPicfightcoin)
	htlcAmt, totalTimelock, hops := generateHops(amount, testStartingHeight,
		n.firstBobChannelLink, n.carolChannelLink)
...
func TestChannelLinkMultiHopDecodeError(t *testing.T) {
	...
	n.carolChannelLink.cfg.ExtractErrorEncrypter = func(
		*pfcec.PublicKey) (ErrorEncrypter, lnwire.FailCode) {
...
func (m *mockPeer) IdentityKey() *pfcec.PublicKey {
	return nil
}
...
func newSingleLinkTestHarness(chanAmt, chanReserve pfcutil.Amount) (
	...
	aliceCfg := ChannelLinkConfig{
		...
		ExtractErrorEncrypter: func(*pfcec.PublicKey) (
...
func TestChannelLinkBandwidthConsistency(t *testing.T) {
	...
	const chanAmt = pfcutil.SatoshiPerPicfightcoin * 5
	...
	// Next, we'll create an HTLC worth 1 PFC, and send it into the link as
...
func TestChannelLinkTrimCircuitsPending(t *testing.T) {
	...
	const (
		chanAmt   = pfcutil.SatoshiPerPicfightcoin * 5
		numHtlcs  = 4
		halfHtlcs = numHtlcs / 2
	)

	// We'll start by creating a new link with our chanAmt (5 PFC). We will
	...
	// Next, we'll create an HTLC worth 1 PFC that will be used as a dummy
...
func TestChannelLinkBandwidthChanReserve(t *testing.T) {
	...
	// channel reserve.
	const chanAmt = pfcutil.SatoshiPerPicfightcoin * 5
	const chanReserve = pfcutil.SatoshiPerPicfightcoin * 1
	...
	// Next, we'll create an HTLC worth 3 PFC, and send it into the link as
	// a switch initiated payment.  The resulting bandwidth should
	// now be decremented to reflect the new HTLC.
	htlcAmt := lnwire.NewMSatFromSatoshis(3 * pfcutil.SatoshiPerPicfightcoin)
	invoice, htlc, err := generatePayment(htlcAmt, htlcAmt, 5, mockBlob)
	if err != nil {
		t.Fatalf("unable to create payment: %v", err)
	}
	...
	const bobChanAmt = pfcutil.SatoshiPerPicfightcoin * 1
	const bobChanReserve = pfcutil.SatoshiPerPicfightcoin * 1.5
