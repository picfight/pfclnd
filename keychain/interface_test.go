...
import (
	...
	_ "github.com/picfight/pfcwallet/walletdb/bdb" // Required in order to create the default database.
)
...
func createTestPfcWallet(coinType uint32) (func(), *wallet.Wallet, error) {
...
func TestKeyRingDerivation(t *testing.T) {
	...
	keyRingImplementations := []keyRingConstructor{
		func() (string, func(), KeyRing, error) {
			cleanUp, wallet, err := createTestPfcWallet(
				CoinTypePicfightcoin,
			)
			if err != nil {
				t.Fatalf("unable to create wallet: %v", err)
			}

			keyRing := NewBtcWalletKeyRing(wallet, CoinTypePicfightcoin)

			return "pfcwallet", cleanUp, keyRing, nil
		},
		func() (string, func(), KeyRing, error) {
			cleanUp, wallet, err := createTestPfcWallet(
				CoinTypeLitecoin,
