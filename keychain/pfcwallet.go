...
const (
	// CoinTypePicfightcoin specifies the BIP44 coin type for Picfightcoin key
	// derivation.
	CoinTypePicfightcoin uint32 = 0
...
// interfaces backed by pfcwallet's internal root waddrmgr. Internally, we'll
...
type BtcWalletKeyRing struct {
	// wallet is a pointer to the active instance of the pfcwallet core.
...
// keychain.SecretKeyRing interface backed by pfcwallet.
//
...
func (b *BtcWalletKeyRing) DerivePrivKey(keyDesc KeyDescriptor) (*pfcec.PrivateKey, error) {
	var key *pfcec.PrivateKey
...
func (b *BtcWalletKeyRing) ScalarMult(keyDesc KeyDescriptor,
	pub *pfcec.PublicKey) ([]byte, error) {
	...
	s := &pfcec.PublicKey{}
	x, y := pfcec.S256().ScalarMult(pub.X, pub.Y, privKey.D.Bytes())
	s.X = x
	s.Y = y
