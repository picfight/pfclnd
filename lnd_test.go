...
import (
	...
	"github.com/picfight/pfclnd/lnrpc"
	"github.com/picfight/pfclnd/lntest"
...
func calcStaticFee(numHTLCs int) pfcutil.Amount {
	const (
		...
		htlcWeight   = 172
		feePerKw     = pfcutil.Amount(50 * 1000 / 4)
	)
...
func createPayReqs(ctx context.Context, node *lntest.HarnessNode,
	paymentAmt pfcutil.Amount, numInvoices int) ([]string, [][]byte,
	[]*lnrpc.Invoice, error) {
...
func testOnchainFundRecovery(net *lntest.NetworkHarness, t *harnessTest) {
	...
	skipAndSend := func(nskip int) func(*lntest.HarnessNode) {
		return func(node *lntest.HarnessNode) {
			...
			// Send one PFC to the next P2WKH address.
			ctxt, _ := context.WithTimeout(ctxb, defaultTimeout)
			err = net.SendCoins(
				ctxt, pfcutil.SatoshiPerPicfightcoin, node,
			)
			if err != nil {
	...
	//
	// After, one PFC is sent to both her first external P2WKH and NP2WKH
	// addresses.
	...
	// send another PFC to the subsequent 10th address in each derivation
	// path.
	restoreCheckBalance(2*pfcutil.SatoshiPerPicfightcoin, 1, skipAndSend(9))

	// Check that using a recovery window of 9 does not find the two most
	// recent txns.
	restoreCheckBalance(2*pfcutil.SatoshiPerPicfightcoin, 9, nil)

	// Extending our recovery window to 10 should find the most recent
	// transactions, leaving the wallet with 4 PFC total.
	//
	// After, we will skip 19 more addrs, sending to the 20th address past
	// our last found address, and repeat the same checks.
	restoreCheckBalance(4*pfcutil.SatoshiPerPicfightcoin, 10, skipAndSend(19))

	// Check that recovering with a recovery window of 19 fails to find the
	// most recent transactions.
	restoreCheckBalance(4*pfcutil.SatoshiPerPicfightcoin, 19, nil)

	// Ensure that using a recovery window of 20 succeeds.
	restoreCheckBalance(6*pfcutil.SatoshiPerPicfightcoin, 20, nil)
}
...
func testBasicChannelFunding(net *lntest.NetworkHarness, t *harnessTest) {
	...
	pushAmt := pfcutil.Amount(100000)

	// First establish a channel with a capacity of 0.5 PFC between Alice
...
func testUpdateChannelPolicy(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, pfcutil.SatoshiPerPicfightcoin, carol)
	if err != nil {
		t.Fatalf("unable to send coins to carol: %v", err)
	}
...
func testChannelBalance(net *lntest.NetworkHarness, t *harnessTest) {
	...
	// Open a channel with 0.16 PFC between Alice and Bob, ensuring the
	...
	checkChannelBalance := func(node lnrpc.LightningClient,
		...
		balance := pfcutil.Amount(response.Balance)
		if balance != amount {
	...
	// exactly 0.5 PFC since now state transitions have taken place yet.
...
func testChannelForceClosure(net *lntest.NetworkHarness, t *harnessTest) {
	...
	const (
		chanAmt     = pfcutil.Amount(10e6)
		pushAmt     = pfcutil.Amount(5e6)
		paymentAmt  = 100000
		numInvoices = 6
	)
	...
	defaultCLTV := uint32(defaultPicfightcoinTimeLockDelta)
	...
	for i := 0; i < numInvoices; i++ {
		err = alicePayStream.Send(&lnrpc.SendRequest{
			...
			FinalCltvDelta: defaultPicfightcoinTimeLockDelta,
		})
		if err != nil {
	...
	carolExpectedBalance := pfcutil.Amount(carolStartingBalance) + pushAmt
	if pfcutil.Amount(carolBalResp.ConfirmedBalance) < carolExpectedBalance {
		t.Fatalf("carol's balance is incorrect: expected %v got %v",
...
func testSphinxReplayPersistence(net *lntest.NetworkHarness, t *harnessTest) {
	...
	assertAmountSent := func(amt pfcutil.Amount) {
...
func testMultiHopPayments(net *lntest.NetworkHarness, t *harnessTest) {
	...
	const chanAmt = pfcutil.Amount(100000)
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, pfcutil.SatoshiPerPicfightcoin, dave)
	if err != nil {
...
func testMultiHopSendToRoute(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, pfcutil.SatoshiPerPicfightcoin, net.Bob)
	if err != nil {
...
func testSendToRouteErrorPropagation(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, pfcutil.SatoshiPerPicfightcoin, charlie)
	if err != nil {
...
func testInvoiceSubscriptions(net *lntest.NetworkHarness, t *harnessTest) {
	...
	const chanAmt = pfcutil.Amount(500000)
...
func testMaxPendingChannels(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	carolBalance := pfcutil.Amount(maxPendingChannels) * amount
...
func testGarbageCollectLinkNodes(net *lntest.NetworkHarness, t *harnessTest) {
	...
	// closed.
	_, err = net.Miner.Node.Generate(defaultPicfightcoinTimeLockDelta - defaultCSV)
	if err != nil {
		t.Fatalf("unable to generate blocks: %v", err)
	}
...
func testDataLossProtection(net *lntest.NetworkHarness, t *harnessTest) {
	...
	timeTravel := func(node *lntest.HarnessNode) (func() error,
		*lnrpc.ChannelPoint, int64, error) {
		...
		// We'll first open up a channel between them with a 0.5 PFC
		// value.
		ctxt, _ := context.WithTimeout(ctxb, channelOpenTimeout)
		chanPoint := openChannelAndAssert(
			...
			for _, chanUpdate := range graphUpdate.ChannelUpdates {
				...
				if chanUpdate.Capacity != int64(chanAmt) {
					t.Fatalf("channel capacities mismatch:"+
						...
						pfcutil.Amount(chanUpdate.Capacity))
				}
				numChannelUpds++
			}
...
func testAsyncPayments(net *lntest.NetworkHarness, t *harnessTest) {
	...
	ctxt, _ := context.WithTimeout(ctxb, channelOpenTimeout)
	channelCapacity := pfcutil.Amount(paymentAmt * 2000)
	chanPoint := openChannelAndAssert(
		ctxt, t, net, net.Alice, net.Bob,
		lntest.OpenChannelParams{
			Amt: channelCapacity,
	...
	availableBalance := pfcutil.Amount(info.LocalBalance) - chanReserve
...
func testMultiHopHtlcLocalTimeout(net *lntest.NetworkHarness, t *harnessTest) {
	...
	const (
		dustHtlcAmt    = pfcutil.Amount(100)
		htlcAmt        = pfcutil.Amount(30000)
		finalCltvDelta = 40
	)
...
func testMultiHopReceiverChainClaim(net *lntest.NetworkHarness, t *harnessTest) {
	...
	numBlocks := uint32(defaultPicfightcoinTimeLockDelta - (2 * defaultBroadcastDelta))
	if _, err := net.Miner.Node.Generate(numBlocks); err != nil {
...
func testMultiHopHtlcRemoteChainClaim(net *lntest.NetworkHarness, t *harnessTest) {
	...
	numBlocks := uint32(defaultPicfightcoinTimeLockDelta-claimDelta) - defaultCSV
	if _, err := net.Miner.Node.Generate(numBlocks); err != nil {
		t.Fatalf("unable to generate blocks")
	}
...
func testSwitchCircuitPersistence(net *lntest.NetworkHarness, t *harnessTest) {
	...
	const chanAmt = pfcutil.Amount(1000000)
	const pushAmt = pfcutil.Amount(900000)
	var networkChans []*lnrpc.ChannelPoint
...
func testSendUpdateDisableChannel(net *lntest.NetworkHarness, t *harnessTest) {
	...
	// Give Eve some coins.
	ctxt, _ = context.WithTimeout(ctxb, defaultTimeout)
	err = net.SendCoins(ctxt, pfcutil.SatoshiPerPicfightcoin, eve)
	if err != nil {
	...
	expectedPolicy := &lnrpc.RoutingPolicy{
		FeeBaseMsat:      int64(defaultPicfightcoinBaseFeeMSat),
		FeeRateMilliMsat: int64(defaultPicfightcoinFeeRate),
...
func TestLightningNetworkDaemon(t *testing.T) {
	...
	// First create an instance of the pfcd's rpctest.Harness. This will be
	...
	handlers := &rpcclient.NotificationHandlers{
		OnTxAccepted: func(hash *chainhash.Hash, amt pfcutil.Amount) {
			lndHarness.OnTxAccepted(hash)
		},
	}
	pfcdHarness, err := rpctest.New(harnessNetParams, handlers, args)
	if err != nil {
		ht.Fatalf("unable to create mining node: %v", err)
	}
	defer func() {
		pfcdHarness.TearDown()

		// After shutting down the chain backend, we'll make a copy of
		// the log file before deleting the temporary log dir.
		logFile := logDir + "/" + harnessNetParams.Name + "/pfcd.log"
		err := lntest.CopyFile("./output_pfcd_chainbackend.log",
			logFile)
		if err != nil {
	...
	lndHarness, err = lntest.NewNetworkHarness(pfcdHarness)
	if err != nil {
	...
	if err := pfcdHarness.SetUp(true, 50); err != nil {
		ht.Fatalf("unable to set up mining node: %v", err)
	}
	if err := pfcdHarness.Node.NotifyNewTransactions(false); err != nil {
	...
	if _, err := pfcdHarness.Node.Generate(numBlocks); err != nil {
		ht.Fatalf("unable to generate blocks: %v", err)
	}

	// With the pfcd harness created, we can now complete the
