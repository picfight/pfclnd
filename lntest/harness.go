...
// node's wallets will be funded wallets with ten 1 PFC outputs each. Finally
...
func (n *NetworkHarness) SetUp(lndArgs []string) error {
	...
	// Load up the wallets of the seeder nodes with 10 outputs of 1 PFC
	// each.
	...
	for _, client := range clients {
		for i := 0; i < 10; i++ {
			...
			addr, err := pfcutil.DecodeAddress(resp.Address, n.netParams)
			if err != nil {
				return err
			}
	...
	expectedBalance := int64(pfcutil.SatoshiPerPicfightcoin * 10)
...
// txWatchRequest encapsulates a request to the harness' Picfightcoin network
...
// transaction once its seen on the Picfightcoin network.
...
func (n *NetworkHarness) OpenPendingChannel(ctx context.Context,
	srcNode, destNode *HarnessNode, amt pfcutil.Amount,
	pushAmt pfcutil.Amount) (*lnrpc.PendingUpdate, error) {
...
func (n *NetworkHarness) SendCoins(ctx context.Context, amt pfcutil.Amount,
...
func (n *NetworkHarness) SendCoinsUnconfirmed(ctx context.Context,
	amt pfcutil.Amount, target *HarnessNode) error {
