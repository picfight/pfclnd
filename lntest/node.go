...
func (cfg nodeConfig) genArgs() []string {
	...
	args = append(args, fmt.Sprintf("--pfcd.rpchost=%v", cfg.RPCConfig.Host))
	args = append(args, fmt.Sprintf("--pfcd.rpcuser=%v", cfg.RPCConfig.User))
	args = append(args, fmt.Sprintf("--pfcd.rpcpass=%v", cfg.RPCConfig.Pass))
	args = append(args, fmt.Sprintf("--pfcd.rawrpccert=%v", encodedCert))
...
func (hn *HarnessNode) WaitForBalance(expectedBalance int64, confirmed bool) error {
	...
	var lastBalance pfcutil.Amount
	doesBalanceMatch := func() bool {
		...
		if confirmed {
			lastBalance = pfcutil.Amount(balance.ConfirmedBalance)
			return balance.ConfirmedBalance == expectedBalance
		}

		lastBalance = pfcutil.Amount(balance.UnconfirmedBalance)
