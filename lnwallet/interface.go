...
// such as: uspv, pfcwallet, Picfightcoin Core, Electrum, etc. This interface then
...
type WalletController interface {
	...
	ConfirmedBalance(confs int32) (pfcutil.Amount, error)
	...
	// p2wsh, etc.
	NewAddress(addrType AddressType, change bool) (pfcutil.Address, error)

	// IsOurAddress checks if the passed address belongs to this wallet
	IsOurAddress(a pfcutil.Address) bool

	// SendOutputs funds, signs, and broadcasts a Picfightcoin transaction paying
	...
	// then finally broadcasts the passed transaction to the Picfightcoin network.
	...
	// which could be e.g. pfcd, picfightcoind, neutrino, or another consensus
	// service.
	BackEnd() string
}
...
type MessageSigner interface {
	...
	SignMessage(pubKey *pfcec.PublicKey, msg []byte) (*pfcec.Signature, error)
}
...
type WalletDriver struct {
	...
	// wallet driver. This could be e.g. picfightcoind, pfcd, neutrino, etc.
