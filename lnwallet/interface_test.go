...
var (
	...
	_, alicePub = pfcec.PrivKeyFromBytes(pfcec.S256(), testHdSeed[:])
	_, bobPub   = pfcec.PrivKeyFromBytes(pfcec.S256(), bobsPrivKey)
...
func calcStaticFee(numHTLCs int) pfcutil.Amount {
	const (
		commitWeight = pfcutil.Amount(724)
		htlcWeight   = 172
		feePerKw     = pfcutil.Amount(250/4) * 1000
	)
	return feePerKw * (commitWeight +
		pfcutil.Amount(htlcWeight*numHTLCs)) / 1000
}
...
func loadTestCredits(miner *rpctest.Harness, w *lnwallet.LightningWallet,
	...
	satoshiPerOutput, err := pfcutil.NewAmount(btcPerOutput)
	if err != nil {
	...
	expectedBalance += pfcutil.Amount(int64(satoshiPerOutput) * int64(numOutputs))
	addrs := make([]pfcutil.Address, 0, numOutputs)
...
func createTestWallet(tempTestDir string, miningNode *rpctest.Harness,
	...
	cfg := lnwallet.Config{
		...
		DefaultConstraints: channeldb.ChannelConstraints{
			DustLimit:        500,
			MaxPendingAmount: lnwire.NewMSatFromSatoshis(pfcutil.SatoshiPerPicfightcoin) * 100,
			ChanReserve:      100,
			MinHTLC:          400,
...
func testDualFundingReservationWorkflow(miner *rpctest.Harness,
	...
	fundingAmount, err := pfcutil.NewAmount(5)
	if err != nil {
		t.Fatalf("unable to create amt: %v", err)
	}

	// In this scenario, we'll test a dual funder reservation, with each
	// side putting in 10 PFC.

	// Alice initiates a channel funded with 5 PFC for each side, so 10 PFC
	// total. She also generates 2 PFC in change.
	...
	// from our HD chain, a change output with 3 PFC, and 2 outputs
	// selected of 4 PFC each. Additionally, the rest of the items needed
...
func testFundingTransactionLockedOutputs(miner *rpctest.Harness,
	...
	// Create a single channel asking for 16 PFC total.
	fundingAmount, err := pfcutil.NewAmount(8)
	if err != nil {
		t.Fatalf("unable to create amt: %v", err)
	}
	feePerKw, err := alice.Cfg.FeeEstimator.EstimateFeePerKW(1)
	...
	// requesting 900 PFC. We only have around 64PFC worth of outpoints
	// that aren't locked, so this should fail.
	amt, err := pfcutil.NewAmount(900)
	if err != nil {
		t.Fatalf("unable to create amt: %v", err)
	}
...
func testFundingCancellationNotEnoughFunds(miner *rpctest.Harness,
	...
	// Create a reservation for 44 PFC.
	fundingAmount, err := pfcutil.NewAmount(44)
	if err != nil {
		t.Fatalf("unable to create amt: %v", err)
	}
	req := &lnwallet.InitFundingReserveMsg{
	...
	// Attempt to create another channel with 44 PFC, this should fail.
...
func testReservationInitiatorBalanceBelowDustCancel(miner *rpctest.Harness,
	...
	const numPFC = 4
	fundingAmount, err := pfcutil.NewAmount(numPFC)
	if err != nil {
		t.Fatalf("unable to create amt: %v", err)
	}

	feePerKw := lnwallet.SatPerKWeight(
		numPFC * numPFC * pfcutil.SatoshiPerPicfightcoin,
	)
	req := &lnwallet.InitFundingReserveMsg{
		ChainHash:       chainHash,
		NodeID:          bobPub,
...
func testSingleFunderReservationWorkflow(miner *rpctest.Harness,
	...
	// First, Alice will Initialize a reservation for a channel with 4 PFC
	// funded solely by us. We'll also initially push 1 PFC of the channel
	// towards Bob's side.
	fundingAmt, err := pfcutil.NewAmount(4)
	if err != nil {
		t.Fatalf("unable to create amt: %v", err)
	}
	pushAmt := lnwire.NewMSatFromSatoshis(pfcutil.SatoshiPerPicfightcoin)
	feePerKw, err := alice.Cfg.FeeEstimator.EstimateFeePerKW(1)
	if err != nil {
		t.Fatalf("unable to query fee estimator: %v", err)
	}
...
func testListTransactionDetails(miner *rpctest.Harness,
	...
	const numTxns = 5
	const outputAmt = pfcutil.SatoshiPerPicfightcoin
	...
	for _, txDetail := range txDetails {
		...
		if txOuts, ok := blockTxOuts[*txDetail.BlockHash][txDetail.Hash]; !ok {
			...
			var destinationAddresses []pfcutil.Address
...
func testPublishTransaction(r *rpctest.Harness,
	...
	pubkeyHash := pfcutil.Hash160(pubKey.PubKey.SerializeCompressed())
	keyAddr, err := pfcutil.NewAddressWitnessPubKeyHash(pubkeyHash,
	...
	txFromOutput := func(tx *wire.MsgTx, payToPubKey *pfcec.PublicKey,
		txFee pfcutil.Amount) *wire.MsgTx {
		// Create a script to pay to.
		payToPubkeyHash := pfcutil.Hash160(payToPubKey.SerializeCompressed())
		payToKeyAddr, err := pfcutil.NewAddressWitnessPubKeyHash(payToPubkeyHash,
			&chaincfg.RegressionNetParams)
		if err != nil {
			t.Fatalf("unable to create addr: %v", err)
		}
	...
	newTx := func() *wire.MsgTx {
		...
		newOutput := &wire.TxOut{
			Value:    pfcutil.SatoshiPerPicfightcoin,
			PkScript: keyScript,
		}
		...
		txFee := pfcutil.Amount(0.1 * pfcutil.SatoshiPerPicfightcoin)
	...
	// into the mempool.
	txFee := pfcutil.Amount(0.05 * pfcutil.SatoshiPerPicfightcoin)
	...
	// as the backing pfcd node will consider the tx being an
	// orphan, and will accept it. Should look into if this is
	// the behavior also for picfightcoind, and update test
	// accordingly.
	...
	// TODO(halseth): test replaceable transactions when pfcd
	// gets RBF support.
}
...
func testSignOutputUsingTweaks(r *rpctest.Harness,
	...
	commitSecret, commitPoint := pfcec.PrivKeyFromBytes(pfcec.S256(),
		commitPreimage)
	...
	baseKey := pubKey
	for i := 0; i < 2; i++ {
		var tweakedKey *pfcec.PublicKey
		if i == 0 {
		...
		pubkeyHash := pfcutil.Hash160(tweakedKey.SerializeCompressed())
		keyAddr, err := pfcutil.NewAddressWitnessPubKeyHash(pubkeyHash,
			&chaincfg.RegressionNetParams)
		if err != nil {
			t.Fatalf("unable to create addr: %v", err)
		...
		vm, err := txscript.NewEngine(keyScript,
			...
			nil, int64(pfcutil.SatoshiPerPicfightcoin))
		if err != nil {
			t.Fatalf("unable to create engine: %v", err)
		}
		if err := vm.Execute(); err != nil {
...
func testReorgWalletBalance(r *rpctest.Harness, w *lnwallet.LightningWallet,
	...
	for i := 0; i < 5; i++ {
		...
		timeout := time.After(30 * time.Second)
		stillConnected := true
		var peers []pfcjson.GetPeerInfoResult
		for stillConnected {
			// Allow for timeout
...
func testChangeOutputSpendConfirmation(r *rpctest.Harness,
	...
	// Assuming a balance of 80 PFC and a transaction fee of 2500 sat/kw,
	...
	if aliceBalance != 0 {
		t.Fatalf("expected alice's balance to be 0 PFC, found %v",
			aliceBalance)
	}

	// Now, we'll send an output back to Alice from Bob of 1 PFC.
	...
	output = &wire.TxOut{
		Value:    pfcutil.SatoshiPerBitcent,
		PkScript: bobPkScript,
	}
	tx = sendCoins(t, r, alice, bob, output, txFeeRate)
	txHash = tx.TxHash()
...
func waitForMempoolTx(r *rpctest.Harness, txid *chainhash.Hash) error {
	...
	for !found {
		...
		if err != nil {
			switch e := err.(type) {
			case *pfcjson.RPCError:
				if e.Code == pfcjson.ErrRPCNoTxInfo {
					continue
				}
			default:
			}
...
func TestLightningWallet(t *testing.T) {
	...
	// Initialize the harness around a pfcd node which will serve as our
	// dedicated miner to generate blocks, cause re-orgs, etc. We'll set
	// up this node with a chain length of 125, so we have plenty of PFC
	// to play around with.
	...
	chainNotifier, err := pfcdnotify.New(&rpcConfig, hintCache, hintCache)
	if err != nil {
...
func runTests(t *testing.T, walletDriver *lnwallet.WalletDriver,
	...
	chainNotifier *pfcdnotify.PfcdNotifier) {
	...
	case "pfcwallet":
		...
		switch backEnd {
		case "pfcd":
			feeEstimator, err = lnwallet.NewPfcdFeeEstimator(
				rpcConfig, 250)
			if err != nil {
				t.Fatalf("unable to create pfcd fee estimator: %v",
					err)
			}
		...
		case "neutrino":
			...
			// instance, and initialize a pfcwallet driver for it.
		...
		case "bitcoind":
			feeEstimator, err = lnwallet.NewPicfightcoindFeeEstimator(
				rpcConfig, 250)
			if err != nil {
				t.Fatalf("unable to create picfightcoind fee estimator: %v",
					err)
			}
			// Start a picfightcoind instance.
			...
			defer os.RemoveAll(tempPicfightcoindDir)
			...
			err = picfightcoind.Start()
			if err != nil {
				t.Fatalf("couldn't start picfightcoind: %v", err)
			}
			defer picfightcoind.Wait()
			defer picfightcoind.Process.Kill()

			// Wait for the picfightcoind instance to start up.
			time.Sleep(time.Second)

			host := fmt.Sprintf("127.0.0.1:%d", rpcPort)
			chainConn, err := chain.NewPicfightcoindConn(
			...
			// Create a pfcwallet picfightcoind client for both Alice and
			// Bob.
			aliceClient = chainConn.NewPicfightcoindClient()
			bobClient = chainConn.NewPicfightcoindClient()
		default:
		...
		aliceWalletConfig := &pfcwallet.Config{
		...
		if err != nil {
			t.Fatalf("unable to create pfcwallet: %v", err)
		}
		aliceSigner = aliceWalletController.(*pfcwallet.BtcWallet)
		aliceKeyRing = keychain.NewBtcWalletKeyRing(
			aliceWalletController.(*pfcwallet.BtcWallet).InternalWallet(),
		...
		bobWalletConfig := &pfcwallet.Config{
		...
		bobSigner = bobWalletController.(*pfcwallet.BtcWallet)
		bobKeyRing = keychain.NewBtcWalletKeyRing(
			bobWalletController.(*pfcwallet.BtcWallet).InternalWallet(),
			keychain.CoinTypeTestnet,
		)
		bio = bobWalletController.(*pfcwallet.BtcWallet)
	default:
		t.Fatalf("unknown wallet driver: %v", walletType)
	}

	// Funding via 20 outputs with 4PFC each.
	...
	// Both wallets should now have 80PFC available for
	// spending.
