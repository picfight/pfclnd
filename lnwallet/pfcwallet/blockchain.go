...
func (b *BtcWallet) GetUtxo(op *wire.OutPoint, pkScript []byte,
	...
	case *chain.RPCClient:
		...
		// We'll ensure we properly convert the amount given in PFC to
		// satoshis.
		amt, err := pfcutil.NewAmount(txout.Value)
		if err != nil {
			return nil, err
		}
	...
	case *chain.PicfightcoindClient:
		...
		// Sadly, gettxout returns the output value in PFC instead of
		// satoshis.
		amt, err := pfcutil.NewAmount(txout.Value)
		if err != nil {
			return nil, err
