...
func senderHTLCScript(senderHtlcKey, receiverHtlcKey,
	revocationKey *pfcec.PublicKey, paymentHash []byte) ([]byte, error) {
	...
	builder.AddData(pfcutil.Hash160(revocationKey.SerializeCompressed()))
...
func senderHtlcSpendRevoke(signer Signer, signDesc *SignDescriptor,
	revokeKey *pfcec.PublicKey, sweepTx *wire.MsgTx) (wire.TxWitness, error) {
...
func receiverHTLCScript(cltvExpiry uint32, senderHtlcKey,
	receiverHtlcKey, revocationKey *pfcec.PublicKey,
...
func createHtlcTimeoutTx(htlcOutput wire.OutPoint, htlcAmt pfcutil.Amount,
	cltvExpiry, csvDelay uint32,
	revocationKey, delayKey *pfcec.PublicKey) (*wire.MsgTx, error) {
...
func createHtlcSuccessTx(htlcOutput wire.OutPoint, htlcAmt pfcutil.Amount,
	csvDelay uint32,
	revocationKey, delayKey *pfcec.PublicKey) (*wire.MsgTx, error) {
...
//  * transition?
//  * covenant output
func secondLevelHtlcScript(revocationKey, delayKey *pfcec.PublicKey,
...
//     OP_ENDIF
//     OP_CHECKSIG
func CommitScriptToSelf(csvTimeout uint32, selfKey, revokeKey *pfcec.PublicKey) ([]byte, error) {
...
func CommitScriptUnencumbered(key *pfcec.PublicKey) ([]byte, error) {
	// This script goes to the "other" party, and it spendable immediately.
	builder := txscript.NewScriptBuilder()
	builder.AddOp(txscript.OP_0)
	builder.AddData(pfcutil.Hash160(key.SerializeCompressed()))
...
func SingleTweakBytes(commitPoint, basePoint *pfcec.PublicKey) []byte {
	h := sha256.New()
...
func TweakPubKey(basePoint, commitPoint *pfcec.PublicKey) *pfcec.PublicKey {
...
func TweakPubKeyWithTweak(pubKey *pfcec.PublicKey, tweakBytes []byte) *pfcec.PublicKey {
	curve := pfcec.S256()
...
//
// Where N is the order of the sub-group.
func TweakPrivKey(basePriv *pfcec.PrivateKey, commitTweak []byte) *pfcec.PrivateKey {
	// tweakInt := sha256(commitPoint || basePub)
	tweakInt := new(big.Int).SetBytes(commitTweak)

	tweakInt = tweakInt.Add(tweakInt, basePriv.D)
	tweakInt = tweakInt.Mod(tweakInt, pfcec.S256().N)

	tweakPriv, _ := pfcec.PrivKeyFromBytes(pfcec.S256(), tweakInt.Bytes())
	return tweakPriv
}
...
func DeriveRevocationPubkey(revokeBase, commitPoint *pfcec.PublicKey) *pfcec.PublicKey {
	...
	//
	// P = R + C
	revX, revY := pfcec.S256().Add(rX, rY, cX, cY)
	return &pfcec.PublicKey{
		X:     revX,
		Y:     revY,
		Curve: pfcec.S256(),
...
//
// Where N is the order of the sub-group.
func DeriveRevocationPrivKey(revokeBasePriv *pfcec.PrivateKey,
	commitSecret *pfcec.PrivateKey) *pfcec.PrivateKey {
	...
	revocationPriv = revocationPriv.Mod(revocationPriv, pfcec.S256().N)

	priv, _ := pfcec.PrivKeyFromBytes(pfcec.S256(), revocationPriv.Bytes())
	return priv
}
...
// for the state.
func ComputeCommitmentPoint(commitSecret []byte) *pfcec.PublicKey {
	x, y := pfcec.S256().ScalarBaseMult(commitSecret)
