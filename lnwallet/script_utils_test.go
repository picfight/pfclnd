...
func TestCommitmentSpendValidation(t *testing.T) {
	...
	const channelBalance = pfcutil.Amount(1 * 10e8)
	const csvTimeout = uint32(5)

	// We also set up set some resources for the commitment transaction.
	// Each side currently has 1 PFC within the channel, with a total
	...
	aliceSelfOutputSigner := &mockSigner{
		privkeys: []*pfcec.PrivateKey{aliceKeyPriv},
	}
	...
	bobSigner := &mockSigner{privkeys: []*pfcec.PrivateKey{bobKeyPriv}}
...
func TestRevocationKeyDerivation(t *testing.T) {
	...
	basePriv, basePub := pfcec.PrivKeyFromBytes(pfcec.S256(),
		testWalletPrivKey)
...
func TestTweakKeyDerivation(t *testing.T) {
	...
	basePriv, basePub := pfcec.PrivKeyFromBytes(pfcec.S256(), baseSecret)
...
func TestHTLCSenderSpendValidation(t *testing.T) {
	...
	bobKeyPriv, bobKeyPub := pfcec.PrivKeyFromBytes(pfcec.S256(),
		bobsPrivKey)
	paymentAmt := pfcutil.Amount(1 * 10e8)
	...
	bobSigner := &mockSigner{privkeys: []*pfcec.PrivateKey{bobKeyPriv}}
	aliceSigner := &mockSigner{privkeys: []*pfcec.PrivateKey{aliceKeyPriv}}
...
func TestSecondLevelHtlcSpends(t *testing.T) {
	...
	// We'll start be creating a creating a 2PFC HTLC.
	const htlcAmt = pfcutil.Amount(2 * 10e8)
	...
	revokePreimage := testHdSeed.CloneBytes()
	commitSecret, commitPoint := pfcec.PrivKeyFromBytes(
		pfcec.S256(), revokePreimage)
