...
func TestTxWeightEstimator(t *testing.T) {
	...
	p2pkhAddr, err := pfcutil.NewAddressPubKeyHash(
	...
	p2wkhAddr, err := pfcutil.NewAddressWitnessPubKeyHash(
		make([]byte, 20), netParams)
	if err != nil {
		t.Fatalf("Failed to generate address: %v", err)
	}
	...
	p2wshAddr, err := pfcutil.NewAddressWitnessScriptHash(
	...
	p2shAddr, err := pfcutil.NewAddressScriptHash([]byte{0}, netParams)
	if err != nil {
		t.Fatalf("Failed to generate address: %v", err)
	}
	...
	for i, test := range testCases {
		...
		expectedWeight := blockchain.GetTransactionWeight(pfcutil.NewTx(tx))
