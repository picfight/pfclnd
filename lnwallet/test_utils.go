...
import (
	...
	"github.com/picfight/pfclnd/shachain"
...
// has just completed.  The channel itself is funded with 10 PFC, with 5 PFC
...
func CreateTestChannels() (*LightningChannel, *LightningChannel, func(), error) {
	channelCapacity, err := pfcutil.NewAmount(10)
	if err != nil {
		return nil, nil, nil, err
	}

	channelBal := channelCapacity / 2
	aliceDustLimit := pfcutil.Amount(200)
	bobDustLimit := pfcutil.Amount(1300)
	...
	var (
		aliceKeys []*pfcec.PrivateKey
		bobKeys   []*pfcec.PrivateKey
	)
	for i := 0; i < 5; i++ {
		...
		aliceKey, _ := pfcec.PrivKeyFromBytes(pfcec.S256(), key)
		...
		bobKey, _ := pfcec.PrivKeyFromBytes(pfcec.S256(), key)
	...
	aliceCommit := channeldb.ChannelCommitment{
		...
		CommitFee:     commitFee,
		FeePerKw:      pfcutil.Amount(feePerKw),
...
func (m *mockSigner) SignOutputRaw(tx *wire.MsgTx, signDesc *SignDescriptor) ([]byte, error) {
	...
	hash160 := pfcutil.Hash160(pubkey.SerializeCompressed())
...
func (m *mockSigner) findKey(needleHash160 []byte, singleTweak []byte,
	doubleTweak *pfcec.PrivateKey) *pfcec.PrivateKey {

	for _, privkey := range m.privkeys {
		// First check whether public key is directly derived from private key.
		hash160 := pfcutil.Hash160(privkey.PubKey().SerializeCompressed())
...
// pubkeyFromHex parses a Picfightcoin public key from a hex encoded string.
func pubkeyFromHex(keyHex string) (*pfcec.PublicKey, error) {
	bytes, err := hex.DecodeString(keyHex)
	if err != nil {
		return nil, err
	}
	return pfcec.ParsePubKey(bytes, pfcec.S256())
}

// privkeyFromHex parses a Picfightcoin private key from a hex encoded string.
func privkeyFromHex(keyHex string) (*pfcec.PrivateKey, error) {
	bytes, err := hex.DecodeString(keyHex)
	if err != nil {
		return nil, err
	}
	key, _ := pfcec.PrivKeyFromBytes(pfcec.S256(), bytes)
	return key, nil

}

// pubkeyToHex serializes a Picfightcoin public key to a hex encoded string.
func pubkeyToHex(key *pfcec.PublicKey) string {
	return hex.EncodeToString(key.SerializeCompressed())
}

// privkeyFromHex serializes a Picfightcoin private key to a hex encoded string.
func privkeyToHex(key *pfcec.PrivateKey) string {
	return hex.EncodeToString(key.Serialize())
}

// signatureFromHex parses a Picfightcoin signature from a hex encoded string.
func signatureFromHex(sigHex string) (*pfcec.Signature, error) {
	bytes, err := hex.DecodeString(sigHex)
	if err != nil {
		return nil, err
	}
	return pfcec.ParseSignature(bytes, pfcec.S256())
}

// blockFromHex parses a full Picfightcoin block from a hex encoded string.
func blockFromHex(blockHex string) (*pfcutil.Block, error) {
	bytes, err := hex.DecodeString(blockHex)
	if err != nil {
		return nil, err
	}
	return pfcutil.NewBlockFromBytes(bytes)
}

// txFromHex parses a full Picfightcoin transaction from a hex encoded string.
func txFromHex(txHex string) (*pfcutil.Tx, error) {
	bytes, err := hex.DecodeString(txHex)
	if err != nil {
		return nil, err
	}
	return pfcutil.NewTxFromBytes(bytes)
}
...
func calcStaticFee(numHTLCs int) pfcutil.Amount {
	const (
		...
		htlcWeight   = 172
		feePerKw     = pfcutil.Amount(24/4) * 1000
	)
