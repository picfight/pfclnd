...
type testContext struct {
	...
	block1    *pfcutil.Block

	fundingInputPrivKey *pfcec.PrivateKey
	localFundingPrivKey *pfcec.PrivateKey
	localPaymentPrivKey *pfcec.PrivateKey

	remoteFundingPubKey    *pfcec.PublicKey
	localFundingPubKey     *pfcec.PublicKey
	localRevocationPubKey  *pfcec.PublicKey
	localPaymentPubKey     *pfcec.PublicKey
	remotePaymentPubKey    *pfcec.PublicKey
	localDelayPubKey       *pfcec.PublicKey
	commitmentPoint        *pfcec.PublicKey
	localPaymentBasePoint  *pfcec.PublicKey
	remotePaymentBasePoint *pfcec.PublicKey

	fundingChangeAddress pfcutil.Address
	fundingInputUtxo     *Utxo
	fundingInputTxOut    *wire.TxOut
	fundingTx            *pfcutil.Tx
...
func newTestContext() (tc *testContext, err error) {
	...
	tc.fundingChangeAddress, err = pfcutil.DecodeAddress(
...
func TestCommitmentAndHTLCTransactions(t *testing.T) {
	...
	var (
		identityKey         *pfcec.PublicKey
		localDelayBasePoint *pfcec.PublicKey
	)
	generateKeys := []**pfcec.PublicKey{
		&identityKey,
		&localDelayBasePoint,
	}
