...
type ErrInsufficientFunds struct {
	amountAvailable pfcutil.Amount
	amountSelected  pfcutil.Amount
}
...
// LightningWallet is a domain specific, yet general Picfightcoin wallet capable of
...
// general purpose Picfightcoin wallet within it. Therefore, it is also able to
// serve as a regular Picfightcoin wallet which uses HD keys. The wallet is highly
...
//
// NOTE: At the moment the wallet requires a pfcd full node, as it's dependent
// on pfcd's websockets notifications as event triggers during the lifetime of a
...
// Picfightcoin Core + ZeroMQ, etc. Eventually, the wallet won't require a full-node
// at all, as SPV support is integrated into pfcwallet.
...
// version of the commitment transaction.
func CreateCommitmentTxns(localBalance, remoteBalance pfcutil.Amount,
	ourChanCfg, theirChanCfg *channeldb.ChannelConfig,
	localCommitPoint, remoteCommitPoint *pfcec.PublicKey,
	...
	otxn := pfcutil.NewTx(ourCommitTx)
	...
	ttxn := pfcutil.NewTx(theirCommitTx)
...
func (l *LightningWallet) handleFundingCounterPartySigs(msg *addCounterPartySigsMsg) {
	...
	sig, err := pfcec.ParseSignature(theirCommitSig, pfcec.S256())
	if err != nil {
		msg.err <- err
		msg.completeChan <- nil
		return
...
func (l *LightningWallet) handleSingleFunderSigs(req *addSingleFunderSigsMsg) {
	...
	// Verify that we've received a valid signature from the remote party
	// for our version of the commitment transaction.
	sig, err := pfcec.ParseSignature(req.theirCommitmentSig, pfcec.S256())
	if err != nil {
		req.err <- err
		req.completeChan <- nil
		return
...
func (l *LightningWallet) selectCoinsAndChange(feeRate SatPerKWeight,
	amt pfcutil.Amount, minConfs int32,
...
func DeriveStateHintObfuscator(key1, key2 *pfcec.PublicKey) [StateHintSize]byte {
	h := sha256.New()
...
// change+fees.
func selectInputs(amt pfcutil.Amount, coins []*Utxo) (pfcutil.Amount, []*Utxo, error) {
	satSelected := pfcutil.Amount(0)
...
// function properly.
func coinSelect(feeRate SatPerKWeight, amt pfcutil.Amount,
	coins []*Utxo) ([]*Utxo, pfcutil.Amount, error) {
