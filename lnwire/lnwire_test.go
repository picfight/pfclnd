...
func randPubKey() (*pfcec.PublicKey, error) {
...
func TestLightningWireProtocol(t *testing.T) {
	...
	customTypeGen := map[MessageType]func([]reflect.Value, *rand.Rand){
		...
		MsgOpenChannel: func(v []reflect.Value, r *rand.Rand) {
			req := OpenChannel{
				FundingAmount:    pfcutil.Amount(r.Int63()),
				PushAmount:       MilliSatoshi(r.Int63()),
				DustLimit:        pfcutil.Amount(r.Int63()),
				MaxValueInFlight: MilliSatoshi(r.Int63()),
				ChannelReserve:   pfcutil.Amount(r.Int63()),
		...
		MsgClosingSigned: func(v []reflect.Value, r *rand.Rand) {
			req := ClosingSigned{
				FeeSatoshis: pfcutil.Amount(r.Int63()),
			}
			var err error
		...
		MsgChannelAnnouncement: func(v []reflect.Value, r *rand.Rand) {
			...
			req.PicfightcoinSig1, err = NewSigFromSignature(testSig)
			if err != nil {
				t.Fatalf("unable to parse sig: %v", err)
				return
			}
			req.PicfightcoinSig2, err = NewSigFromSignature(testSig)
			if err != nil {
				t.Fatalf("unable to parse sig: %v", err)
				return
			}
			...
			req.PicfightcoinKey1, err = randRawKey()
			if err != nil {
				t.Fatalf("unable to generate key: %v", err)
				return
			}
			req.PicfightcoinKey2, err = randRawKey()
			if err != nil {
				t.Fatalf("unable to generate key: %v", err)
				return
			}
		...
		MsgAnnounceSignatures: func(v []reflect.Value, r *rand.Rand) {
			...
			req.PicfightcoinSignature, err = NewSigFromSignature(testSig)
			if err != nil {
				t.Fatalf("unable to parse sig: %v", err)
				return
			}
