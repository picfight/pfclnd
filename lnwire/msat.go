...
// of satoshis.
func NewMSatFromSatoshis(sat pfcutil.Amount) MilliSatoshi {
	return MilliSatoshi(uint64(sat) * mSatScale)
}

// ToPFC converts the target MilliSatoshi amount to its corresponding value
// when expressed in PFC.
func (m MilliSatoshi) ToPFC() float64 {
	sat := m.ToSatoshis()
	return sat.ToPFC()
}
...
func (m MilliSatoshi) ToSatoshis() pfcutil.Amount {
	return pfcutil.Amount(uint64(m) / mSatScale)
}
