...
// Sig is a fixed-sized ECDSA signature. Unlike Picfightcoin, we use fixed sized
// signatures on the wire, instead of DER encoded signatures. This type
// provides several methods to convert to/from a regular Picfightcoin DER encoded
// signature (raw bytes and *pfcec.Signature).
type Sig [64]byte

// NewSigFromRawSignature returns a Sig from a Picfightcoin raw signature encoded in
...
func NewSigFromRawSignature(sig []byte) (Sig, error) {
	...
	// See the Serialize() method for pfcec.Signature for details.
	rLen := sig[3]
	sLen := sig[5+rLen]
...
// existing pfcec.Signature.
func NewSigFromSignature(e *pfcec.Signature) (Sig, error) {
	if e == nil {
...
// ToSignature converts the fixed-sized signature to a pfcec.Signature objects
// which can be used for signature validation checks.
func (b *Sig) ToSignature() (*pfcec.Signature, error) {
	// Parse the signature with strict checks.
	sigBytes := b.ToSignatureBytes()
	sig, err := pfcec.ParseDERSignature(sigBytes, pfcec.S256())
	if err != nil {
		return nil, err
	}
...
// big-endian integer) such that it passes pfcec.canonicalPadding test.
