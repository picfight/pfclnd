...
// interaction with the picfightcoin network.
type mockWalletController struct {
	rootKey               *pfcec.PrivateKey
	prevAddres            pfcutil.Address
...
func (*mockWalletController) FetchInputInfo(
	...
	txOut := &wire.TxOut{
		Value:    int64(10 * pfcutil.SatoshiPerPicfightcoin),
		PkScript: []byte("dummy"),
	}
	return txOut, nil
}
func (*mockWalletController) ConfirmedBalance(confs int32) (pfcutil.Amount, error) {
	return 0, nil
}

// NewAddress is called to get new addresses for delivery, change etc.
func (m *mockWalletController) NewAddress(addrType lnwallet.AddressType,
	change bool) (pfcutil.Address, error) {
	addr, _ := pfcutil.NewAddressPubKey(
		m.rootKey.PubKey().SerializeCompressed(), &chaincfg.MainNetParams)
	return addr, nil
}
func (*mockWalletController) IsOurAddress(a pfcutil.Address) bool {
	return false
}
...
func (m *mockWalletController) ListUnspentWitness(minconfirms,
	...
	utxo := &lnwallet.Utxo{
		...
		Value:       pfcutil.Amount(10 * pfcutil.SatoshiPerPicfightcoin),
...
func (m *mockSecretKeyRing) DerivePrivKey(keyDesc keychain.KeyDescriptor) (*pfcec.PrivateKey, error) {
	return m.rootKey, nil
}

func (m *mockSecretKeyRing) ScalarMult(keyDesc keychain.KeyDescriptor,
	pubKey *pfcec.PublicKey) ([]byte, error) {
	return nil, nil
}
