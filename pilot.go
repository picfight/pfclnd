...
func (c *chanController) OpenChannel(target *pfcec.PublicKey,
...
func (c *chanController) SpliceIn(chanPoint *wire.OutPoint,
	amt pfcutil.Amount) (*autopilot.Channel, error) {
	return nil, nil
}
...
func initAutoPilot(svr *server, cfg *autoPilotConfig) *autopilot.ManagerCfg {
	...
	atplConstraints := &autopilot.HeuristicConstraints{
		MinChanSize:     pfcutil.Amount(cfg.MinChannelSize),
		MaxChanSize:     pfcutil.Amount(cfg.MaxChannelSize),
	...
	pilotCfg := autopilot.Config{
		...
		ConnectToPeer: func(target *pfcec.PublicKey, addrs []net.Addr) (bool, error) {
