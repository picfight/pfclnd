...
import (
	...
	_ "github.com/picfight/pfcwallet/walletdb/bdb" // Required to register the boltdb walletdb implementation.
...
var (
	...
	addrPk, _       = pfcutil.NewAddressPubKey(pubKey.SerializeCompressed(),
		netParams)
...
func testFilterSingleBlock(node *rpctest.Harness, chainView FilteredChainView,
	...
	txns := []*pfcutil.Tx{pfcutil.NewTx(spendingTx1), pfcutil.NewTx(spendingTx2)}
...
func testFilterBlockDisconnected(node *rpctest.Harness,
	...
	if err = reorgView.Start(); err != nil {
		t.Fatalf("unable to start pfcd chain view: %v", err)
	}
		...
		chainViewInit: func(_ rpcclient.ConnConfig, p2pAddr string) (func(), FilteredChainView, error) {
			...
			if err != nil {
				return cleanUp2, nil, fmt.Errorf("unable to "+
					"establish connection to picfightcoind: %v",
					err)
			}
			...
			chainView := NewPicfightcoindFilteredChainView(chainConn)
...
func TestFilteredChainView(t *testing.T) {
	...
	// this node with a chain length of 125, so we have plenty of PFC to
	// play around with.
