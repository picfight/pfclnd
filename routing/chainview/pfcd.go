...
// PfcdFilteredChainView is an implementation of the FilteredChainView
// interface which is backed by an active websockets connection to pfcd.
type PfcdFilteredChainView struct {
...
// A compile time check to ensure PfcdFilteredChainView implements the
// chainview.FilteredChainView.
var _ FilteredChainView = (*PfcdFilteredChainView)(nil)

// NewPfcdFilteredChainView creates a new instance of a FilteredChainView from
// RPC credentials for an active pfcd instance.
func NewPfcdFilteredChainView(config rpcclient.ConnConfig) (*PfcdFilteredChainView, error) {
	chainView := &PfcdFilteredChainView{
...
func (b *PfcdFilteredChainView) Start() error {
	// Already started?
	...
	if err := b.pfcdConn.NotifyBlocks(); err != nil {
		return err
	}

	_, bestHeight, err := b.pfcdConn.GetBestBlock()
	if err != nil {
		return err
	}
...
//
// NOTE: This is part of the FilteredChainView interface.
func (b *PfcdFilteredChainView) Stop() error {
...
func (b *PfcdFilteredChainView) onFilteredBlockConnected(height int32,
	header *wire.BlockHeader, txns []*pfcutil.Tx) {
...
func (b *PfcdFilteredChainView) onFilteredBlockDisconnected(height int32,
...
//
// NOTE: This is part of the FilteredChainView interface.
func (b *PfcdFilteredChainView) FilterBlock(blockHash *chainhash.Hash) (*FilteredBlock, error) {
	req := &filterBlockReq{
		blockHash: blockHash,
...
func (b *PfcdFilteredChainView) chainFilterer() {
	defer b.wg.Done()
	...
	decodeJSONBlock := func(block *pfcjson.RescannedBlock,
	...
	for {
		...
		case update := <-b.filterUpdates:
			...
			// Apply the new TX filter to pfcd, which will cause
			// all following notifications from and calls to it
			// return blocks filtered with the new filter.
			b.pfcdConn.LoadTxFilter(false, []pfcutil.Address{},
				update.newUtxos)
			...
			// with pfcd applying the newly loaded filter to each
			// block.
			for i := update.updateHeight + 1; i < bestHeight+1; i++ {
				blockHash, err := b.pfcdConn.GetBlockHash(int64(i))
				if err != nil {
				...
				// have gone missing.
				rescanned, err := b.pfcdConn.RescanBlocks(
		...
		case req := <-b.filterBlockReqs:
			...
			block, err := b.pfcdConn.GetBlock(req.blockHash)
			if err != nil {
				req.err <- err
				req.resp <- nil
				continue
			}
			header, err := b.pfcdConn.GetBlockHeaderVerbose(req.blockHash)
			if err != nil {
				req.err <- err
				req.resp <- nil
				continue
...
//
// NOTE: This is part of the FilteredChainView interface.
func (b *PfcdFilteredChainView) UpdateFilter(ops []channeldb.EdgePoint,
...
//
// NOTE: This is part of the FilteredChainView interface.
func (b *PfcdFilteredChainView) FilteredBlocks() <-chan *FilteredBlock {
...
//
// NOTE: This is part of the FilteredChainView interface.
func (b *PfcdFilteredChainView) DisconnectedBlocks() <-chan *FilteredBlock {
