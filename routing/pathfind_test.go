...
func parseTestGraph(path string) (*testGraphInstance, error) {
	...
	aliasMap := make(map[string]*pfcec.PublicKey)
	...
	for _, node := range g.Nodes {
		...
		pub, err := pfcec.ParsePubKey(pubBytes, pfcec.S256())
		if err != nil {
			return nil, err
		}
	...
	for _, edge := range g.Edges {
		...
		edgeInfo := channeldb.ChannelEdgeInfo{
			...
			Capacity:     pfcutil.Amount(edge.Capacity),
		}

		copy(edgeInfo.NodeKey1Bytes[:], node1Bytes)
		copy(edgeInfo.NodeKey2Bytes[:], node2Bytes)
		copy(edgeInfo.PicfightcoinKey1Bytes[:], node1Bytes)
		copy(edgeInfo.PicfightcoinKey2Bytes[:], node2Bytes)
...
func symmetricTestChannel(alias1 string, alias2 string, capacity pfcutil.Amount,
...
func createTestGraphFromChannels(testChannels []*testChannel) (*testGraphInstance, error) {
	...
	aliasMap := make(map[string]*pfcec.PublicKey)
	privKeyMap := make(map[string]*pfcec.PrivateKey)
	...
	addNodeWithAlias := func(alias string) (*channeldb.LightningNode, error) {
		...
		privKey, pubKey := pfcec.PrivKeyFromBytes(pfcec.S256(),
			keyBytes)
...
func getAliasFromPubKey(pubKey []byte,
	aliases map[string]*pfcec.PublicKey) string {
...
func TestPathFindingWithAdditionalEdges(t *testing.T) {
	...
	dogePubKey, err := pfcec.ParsePubKey(dogePubKeyBytes, pfcec.S256())
	if err != nil {
...
func TestPathNotAvailable(t *testing.T) {
	...
	unknownNode, err := pfcec.ParsePubKey(unknownNodeBytes, pfcec.S256())
	if err != nil {
...
func TestPathInsufficientCapacity(t *testing.T) {
	...
	// To test his we'll attempt to make a payment of 1 PFC, or 100 million
	...
	payAmt := lnwire.NewMSatFromSatoshis(pfcutil.SatoshiPerPicfightcoin)
	_, err = findPath(
		&graphParams{
			graph: graph.graph,
		},
