...
// ChannelRouter is the HtlcSwitch, and below that is the Picfightcoin blockchain
...
func (r *ChannelRouter) processUpdate(msg interface{}) error {
	...
	case *channeldb.ChannelEdgeInfo:
		...
		// edge picfightcoin keys and channel value corresponds to the
		// reality.
		witnessScript, err := lnwallet.GenMultiSigScript(
			msg.PicfightcoinKey1Bytes[:], msg.PicfightcoinKey2Bytes[:],
		)
		if err != nil {
			return err
		}
		...
		msg.Capacity = pfcutil.Amount(chanUtxo.Value)
...
// fee along the route.
func (r *ChannelRouter) FindRoutes(target *pfcec.PublicKey,
...
func generateSphinxPacket(route *Route, paymentHash []byte) ([]byte,
	*sphinx.Circuit, error) {
	...
	// in each hop.
	nodes := make([]*pfcec.PublicKey, len(route.Hops))
	for i, hop := range route.Hops {
		pub, err := pfcec.ParsePubKey(hop.PubKeyBytes[:],
			pfcec.S256())
		if err != nil {
			return nil, nil, err
		}
	...
	sessionKey, err := pfcec.NewPrivateKey(pfcec.S256())
	if err != nil {
		return nil, nil, err
	}
...
func (r *ChannelRouter) FetchLightningNode(node Vertex) (*channeldb.LightningNode, error) {
	pubKey, err := pfcec.ParsePubKey(node[:], pfcec.S256())
	if err != nil {
