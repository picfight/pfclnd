...
import (
	...
	"github.com/picfight/pfclnd/macaroons"
	"github.com/picfight/pfclnd/routing"
	"github.com/picfight/pfclnd/signal"
	"github.com/picfight/pfclnd/zpay32"
...
const (
	// maxBtcPaymentMSat is the maximum allowed Picfightcoin payment currently
...
var (
	...
	// It is set to the value under the Picfightcoin chain as default.
...
func addrPairsToOutputs(addrPairs map[string]int64) ([]*wire.TxOut, error) {
	...
	for addr, amt := range addrPairs {
		addr, err := pfcutil.DecodeAddress(addr, activeNetParams.Params)
		if err != nil {
			return nil, err
		}
...
func (r *rpcServer) SendCoins(ctx context.Context,
	...
	rpcsLog.Infof("[sendcoins] addr=%v, amt=%v, sat/kw=%v", in.Addr,
		pfcutil.Amount(in.Amount), int64(feePerKw))
...
func (r *rpcServer) VerifyMessage(ctx context.Context,
	...
	pubKey, _, err := pfcec.RecoverCompact(pfcec.S256(), sig, digest)
	if err != nil {
...
func (r *rpcServer) ConnectPeer(ctx context.Context,
	...
	pubKey, err := pfcec.ParsePubKey(pubkeyHex, pfcec.S256())
	if err != nil {
		return nil, err
	}
...
func (r *rpcServer) DisconnectPeer(ctx context.Context,
	...
	peerPubKey, err := pfcec.ParsePubKey(pubKeyBytes, pfcec.S256())
	if err != nil {
...
func (r *rpcServer) OpenChannel(in *lnrpc.OpenChannelRequest,
	...
	localFundingAmt := pfcutil.Amount(in.LocalFundingAmount)
	remoteInitialBalance := pfcutil.Amount(in.PushSat)
	...
	nodePubKey, err = pfcec.ParsePubKey(in.NodePubkey, pfcec.S256())
	if err != nil {
		return err
	}
...
func (r *rpcServer) OpenChannelSync(ctx context.Context,
	...
	nodepubKey, err := pfcec.ParsePubKey(keyBytes, pfcec.S256())
	if err != nil {
		return nil, err
	}
...
func (r *rpcServer) ChannelBalance(ctx context.Context,
	...
	var pendingOpenBalance pfcutil.Amount
...
func (r *rpcServer) PendingChannels(ctx context.Context,
	...
	for i, pendingChan := range pendingOpenChannels {
		...
		utx := pfcutil.NewTx(localCommitment.CommitTx)
...
func (r *rpcServer) ListChannels(ctx context.Context,
	...
	for _, dbChannel := range dbChannels {
		...
		utx := pfcutil.NewTx(localCommit.CommitTx)
		commitBaseWeight := blockchain.GetTransactionWeight(utx)
		commitWeight := commitBaseWeight + lnwallet.WitnessCommitmentTxWeight
		...
		var sumOutputs pfcutil.Amount
		for _, txOut := range localCommit.CommitTx.TxOut {
			sumOutputs += pfcutil.Amount(txOut.Value)
		}
...
func calculateFeeLimit(feeLimit *lnrpc.FeeLimit,
	...
	case *lnrpc.FeeLimit_Fixed:
		return lnwire.NewMSatFromSatoshis(
			pfcutil.Amount(feeLimit.GetFixed()),
		)
...
func extractPaymentIntent(rpcPayReq *rpcPaymentRequest) (rpcPaymentIntent, error) {
	...
	if len(rpcPayReq.Dest) != 0 {
		payIntent.dest, err = pfcec.ParsePubKey(
			rpcPayReq.Dest, pfcec.S256(),
		)
		if err != nil {
			return payIntent, err
		}
		...
		payIntent.dest, err = pfcec.ParsePubKey(pubBytes, pfcec.S256())
		if err != nil {
			return payIntent, err
		}
	}
...
func (r *rpcServer) AddInvoice(ctx context.Context,
	...
	amt := pfcutil.Amount(invoice.Value)
	...
	if len(invoice.FallbackAddr) > 0 {
		addr, err := pfcutil.DecodeAddress(invoice.FallbackAddr,
	...
	default:
		...
		defaultDelta := cfg.Picfightcoin.TimeLockDelta
...
func (r *rpcServer) marshallRoute(route *routing.Route) *lnrpc.Route {
	...
	for i, hop := range route.Hops {
		...
		// graph.
		var chanCapacity pfcutil.Amount
...
func (r *rpcServer) GetNetworkInfo(ctx context.Context,
	...
	var (
		...
		numChannels          uint32
		maxChanOut           uint32
		totalNetworkCapacity pfcutil.Amount
		minChannelSize       pfcutil.Amount = math.MaxInt64
		maxChannelSize       pfcutil.Amount
	)
...
func marshallTopologyChange(topChange *routing.TopologyChange) *lnrpc.GraphTopologyUpdate {
	...
	// the public key.
	encodeKey := func(k *pfcec.PublicKey) string {
