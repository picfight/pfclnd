...
func generateInputPartitionings(sweepableInputs []Input,
	...
	dustLimit := txrules.GetDustThreshold(
		lnwallet.P2WPKHSize,
		pfcutil.Amount(relayFeePerKW.FeePerKVByte()),
	)
...
// minimizing any negative externalities we cause for the Picfightcoin system as a
// whole.
func getPositiveYieldInputs(sweepableInputs []Input, maxInputs int,
	feePerKW lnwallet.SatPerKWeight) (int, pfcutil.Amount) {
	...
	var total, outputValue pfcutil.Amount
	for idx, input := range sweepableInputs {
		...
		newTotal := total + pfcutil.Amount(input.SignDesc().Output.Value)
...
func createSweepTx(inputs []Input, outputPkScript []byte,
	...
	var totalSum pfcutil.Amount
	for _, o := range inputs {
		totalSum += pfcutil.Amount(o.SignDesc().Output.Value)
	}
