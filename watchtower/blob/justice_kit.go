...
func (b *JusticeKit) CommitToLocalWitnessScript() ([]byte, error) {
	revocationPubKey, err := pfcec.ParsePubKey(
		b.RevocationPubKey[:], pfcec.S256(),
	)
	if err != nil {
		return nil, err
	}

	localDelayedPubKey, err := pfcec.ParsePubKey(
		b.LocalDelayPubKey[:], pfcec.S256(),
	)
	if err != nil {
		return nil, err
	}
...
func (b *JusticeKit) HasCommitToRemoteOutput() bool {
	return pfcec.IsCompressedPubKey(b.CommitToRemotePubKey[:])
}
...
func (b *JusticeKit) CommitToRemoteWitnessScript() ([]byte, error) {
	if !pfcec.IsCompressedPubKey(b.CommitToRemotePubKey[:]) {
...
func (b *JusticeKit) decodeV0(r io.Reader) error {
	...
	if pfcec.IsCompressedPubKey(commitToRemotePubkey[:]) {
