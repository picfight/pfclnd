...
const (
	// mSatPerBtc is the number of millisatoshis in 1 PFC.
...
type MessageSigner struct {
	...
	// format returned by pfcec.SignCompact.
...
func Destination(destination *pfcec.PublicKey) func(*Invoice) {
	return func(i *Invoice) {
...
func FallbackAddr(fallbackAddr pfcutil.Address) func(*Invoice) {
	return func(i *Invoice) {
...
func Decode(invoice string, net *chaincfg.Params) (*Invoice, error) {
	...
	if decodedInvoice.Destination != nil {
		...
		pubkey, _, err := pfcec.RecoverCompact(pfcec.S256(),
			compactSign, hash)
		if err != nil {
			return nil, err
		}
...
func parseDestination(data []byte) (*pfcec.PublicKey, error) {
	...
	return pfcec.ParsePubKey(base256Data, pfcec.S256())
}
...
// on-chain address.
func parseFallbackAddr(data []byte, net *chaincfg.Params) (pfcutil.Address, error) {
	...
	case 0:
		...
		switch len(witness) {
		case 20:
			addr, err = pfcutil.NewAddressWitnessPubKeyHash(witness, net)
		case 32:
			addr, err = pfcutil.NewAddressWitnessScriptHash(witness, net)
		default:
	...
	case 17:
		...
		addr, err = pfcutil.NewAddressPubKeyHash(pubKeyHash, net)
		if err != nil {
			return nil, err
		}
	case 18:
		...
		addr, err = pfcutil.NewAddressScriptHashFromHash(scriptHash, net)
		if err != nil {
			return nil, err
		}
	default:
...
func parseRouteHint(data []byte) ([]routing.HopHint, error) {
	...
	for len(base256Data) > 0 {
		...
		hopHint.NodeID, err = pfcec.ParsePubKey(base256Data[:33], pfcec.S256())
		if err != nil {
			return nil, err
		}
